...
extern "C" int mainlaszip(int argc, char *argv[])
{
  laszip_U32 i;
...
static void closeFileNum(int fileNum, LPLAZFILESTRUCT pFiles)
{
	if (pFiles->lastUse[fileNum])
	{
		laszip_close_reader(pFiles->laszip_readers[fileNum]);
		laszip_destroy(pFiles->laszip_readers[fileNum]);
		pFiles->lastUse[fileNum] = 0;
	}
}
static int getFileNum(int fileNum, char * file, LPLAZFILESTRUCT pFiles)
{
	int ifile = 0;
	__int64 minUse = LLONG_MAX;

	for (int i = 0; i < 4; i++)
	{
		if (pFiles->lazFileNums[i] == fileNum)
			return i;
	}
	for (int i = 0; i < 4; i++)
	{
		if (pFiles->lastUse[i] < minUse)
		{
			ifile = i;
			minUse = pFiles->lastUse[i];
		}
	}
	pFiles->lazFileNums[ifile] = fileNum;
	strcpy(pFiles->lazFiles[ifile], file);
	closeFileNum(ifile, pFiles);
	laszip_create(&pFiles->laszip_readers[ifile]);
	laszip_exploit_spatial_index(pFiles->laszip_readers[ifile], 1);
	laszip_BOOL is_compressed = 0;
	laszip_open_reader(pFiles->laszip_readers[ifile], file, &is_compressed);
	return ifile;
}
extern "C" int getLAZPointsInBounds(LPLAZFILESTRUCT pFiles,int fileNum,char * file, int wantType, LPMNMXCORD pBounds, int maxPoints, LPDPOINT3D points)
{
	int rtn=0;
	laszip_point* point;
	...
	if (laszip_load_dll() != 1)
	{
		int ifile = getFileNum(fileNum, file, pFiles);
		pFiles->lastUse[ifile] = use++;
		laszip_POINTER laszip_reader = pFiles->laszip_readers[ifile];
		{
			{
				{
					laszip_BOOL is_empty = 0;
					if (!laszip_inside_rectangle(laszip_reader, pBounds->xmn, pBounds->ymn, pBounds->xmx, pBounds->ymx,&is_empty))
					{
						laszip_header* header;
