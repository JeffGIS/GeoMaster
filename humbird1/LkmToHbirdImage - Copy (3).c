...
PLKMTILE GetLKMImageTile (POINT *Point,int iScale,LPIPILOTSTRUCT piPilot,LPINT pRc);
...
Top:
	...
	while ( !bIsAbort && (pLKMTile = GetLKMImageTile (&TilePoint,ForceLevel,piPilot,pRc)))
	{
		nTiles++;
...
BOOL GetAdjoiningTile (LPPOINT pTilePt,LPINT pcornerIndex,int iScale,PLKMTILE *pLKMTile,LPIPILOTSTRUCT piPilot)
{
...
StartTracking:
	...
	GetLKMImageTileFromScaleAndID (-1,0,0,0);
	{
...
SplitRing:
			...
			while (foundPixel && TilePointOnScreen (nextPoint,pLKMTile) && GetAdjoiningTile (&nextPoint,&cornerIndex,piPilot->iScale,&pLKMTile,piPilot));
...
PLKMTILE GetLKMImageTile (LPPOINT pTilePoint,int iScale,LPIPILOTSTRUCT piPilot,LPINT pRc)
{
	PLKMTILE pLKMTile;
...
PLKMTILE GetLKMImageTileFromScaleAndID (int iScale,int CellID,LPIPILOTSTRUCT piPilot,LPINT pRc)
{
	...
	int	len, storedlen, compressedlen;
	static	int	nRead=0;
	int	ID, NextID, LastID,i,j;
	int	loc=-1,pos,block,ioff,mini;
	unsigned int	minUse;
...
static	unsigned int lastUse=0;
	...
	for (i=0;i<MAXBUFFEREDTILES;i++)
	{
		if (pLKMTileBuffer[i])
		{
			if (pLKMTileBuffer[i]->CellID == CellID && pLKMTileBuffer[i]->iScale == iScale)
			{
...
NextFile:
	...
	for (i=j;i<MAXBUFFEREDTILES;i++)
	{
		if (pLKMTileBuffer[i] == NULL)
		{
			...
			pLKMTileBuffer[i]->iScale = iScale;
			pLKMTileBuffer[i]->lastUse = lastUse++;
			return pLKMTile;
		}
