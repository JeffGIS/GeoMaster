...
#define MAX_CHARTOBJECT_SCALE	20
#define MAX_CHARTTEXT_SCALE		20
#define MAX_CONTOURLINE_SCALE	4
...
NextCTextFile:
	...
	if (!bIsAbort && Scale < MAX_CHARTOBJECT_SCALE) //process chart objects
	{
...
/*	if (!bIsAbort)
	{
		if (LKMBeginEnumObjects (CenterX,CenterY,RangeInMeters,Scale, 5,&enumHandle ))
		{
			while(LKMEnumNextObject( enumHandle, &ObjectHandle  ))
			{
              ...
              ClChartRenderCb pfCallBack, void *pCallBackData )
{
	...
	if (HighlightLMLakes || *pScale > MAX_CONTOURLINE_SCALE)
	...
	for (i=0;i<CTextFileHeader.NumIndexLevs;i++)
	{
		...
		for (pos=0;pos<CTextFileHeader.NumPerIndexRec;pos++)
		{
			if (CellID < IndexRecordBlock[pos].GridCellID)
			{
				if (!pos)
				{
					CellID = IndexRecordBlock[pos].GridCellID - 1;
					*pTilePoint = GetGridPoint (CellID);
					goto Top;
				}
	...
	for (i=0;i<pEnumObject->ObjectFileHeader.NumIndexLevs;i++)
	{
		...
		for (pos=0;pos<pEnumObject->ObjectFileHeader.NumPerIndexRec;pos++)
		{
			if (CellID < IndexRecordBlock[pos].GridCellID)
			{
				...
				{
					CellID = IndexRecordBlock[pos].GridCellID - 1;
					pEnumObject->TilePoint = GetGridPoint (CellID);
					goto Top;
				}
				NextID = IndexRecordBlock[pos].GridCellID;
				break;
