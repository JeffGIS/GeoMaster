...
#ifdef DEBUG
// #define LKMTRACE   // Make sure that the CBUG_PORT is configured properly in Option.h if LKMTRACE is defined.
// For M1197 model, CBUG_PORT should be UART2
#elif defined(linux)
// #define LKMTRACE
#endif
...
static unsigned int   MAX_READ_BUFFER_INDEX_ENTRIES=1024;
static unsigned int   READ_BUFFER_SIZE=(1024*1024);
   
// #ifdef ENABLE_LKM_BUFFERING
   // static unsigned   int   MAX_READ_BUFFER_INDEX_ENTRIES=1024;
   // static unsigned int     READ_BUFFER_SIZE=(1024*1024);
// #else
   // static unsigned   int   MAX_READ_BUFFER_INDEX_ENTRIES=0;
   // static unsigned int     READ_BUFFER_SIZE=0;
// #endif
...
typedef struct
{
   char c[IMAGETILESIZE_ALIGNED];
}IMAGETILEBUFFERELEMENT;
typedef IMAGETILEBUFFERELEMENT *LPIMAGETILEBUFFERELEMENT;

#ifdef ENABLE_LKM_BUFFERING
static   int   MAXBUFFEREDIMAGETILES=42;
#else
static   int   MAXBUFFEREDIMAGETILES=0;
#endif
static	int	InitialImageTileSize;
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
static	int	maxSelectionTrack=0, maxSelectedContour=0;
static	double selectionTrackSpacing=1.0;
#endif

static	int	cpBlockingSize=5;
...
typedef struct
{
   long GridID,GridMinX,GridMinY,GridMaxX,GridMaxY,GridWidth,GridHeight,nGridCol,nGridRow;
...
static int	isFTCChip=0;
static int  LZOerror=0;
...
void AllocateImageTileBuffer (int size);
...
int DecompressBinaryRecord (LPBYTE pDecompressedRec,int DecompressedLen,LPBYTE pCompressedRec,int CompressedLen);
int DeCompressByteArray (LPBYTE pMemCmp,int lMem,LPBYTE pDestMem,int DestLen);
CL_BOOL DecompressTile (LPBYTE Image,int DecompressedLen, LPBYTE CompressedImage, int compressedlen);
...
void InitReadBuffer (BOOL Close)
{
	...
	else if (first)
	{
		...
		ReadBufferLength = 0;
		first = FALSE;
                LKMTrace ("InitReadBuffer: Allocating memroy for LM buffers");
...
int BufferedRead (CLFILE *Fid,int iFile,int lRead,LPBYTE pData)
{
	...
	if (nInBuffer >= MAX_READ_BUFFER_INDEX_ENTRIES || ReadBufferLength + (unsigned int)lRead >= READ_BUFFER_SIZE)
   {
   ...
   rtn = pReadBufferIndex[nInBuffer].lReadActual = cl_fread (pData,1,lRead,Fid);
...
CLFILE *db_fopen2( char *filename, const char *mode )
{
	...
	{
		unsigned int i;
		sprintf (LKMTRACESTRING,"RBS=%i,COF=%i,NOF=%i",READ_BUFFER_SIZE,CurrentOpenFile,NumOpenedFiles);
		LKMTrace (LKMTRACESTRING);
		for (i=0;i<NumOpenedFiles;i++)
		{
			sprintf (LKMTRACESTRING,"%s,%i,%i",OpenedFiles[i],FilePos[i],FileLength[i]);
			LKMTrace (LKMTRACESTRING);
		}
	}
#endif
	...
	for (i=0;i<nInBuffer;i++)
	{
		if (pReadBufferIndex[i].iFile == CurrentOpenFile)
			pReadBufferIndex[i].iFile = -1;
	}
   ...
   Exit:
	...
	if (tracefopen)
	{
		sprintf (LKMTRACESTRING,"Return from dbfopen2,COF=%i,NOF=%i",CurrentOpenFile,NumOpenedFiles);
		LKMTrace (LKMTRACESTRING);
	}
#endif
    return fid;
...
void SortFileTimes (void)
{
	...
	{
		int	slowesttim=-1;
		unsigned int	slowesti = NumOpenedFiles;
...
int GridCellID (LPCL_POINT pPoint)
{
	...
	Col = (int) ((pPoint->x) - GridMinX) / GridWidth;
	Row = (int) ((pPoint->y) - GridMinY) / GridHeight;
	if (Col < 0 || Row < 0)
		return -1;
...
CL_BOOL LKMGetCardName (char *PathToLKMData,char * CardName, int MaxLen )
{
   ...
   Fid = db_fopen (File,"rb");
	if (!Fid)
		return 0;
   db_fread2 (&setupdata,5,sizeof(setupdata),Fid, CL_FALSE);
...
int LKMToHBInit (char *PathToLKMData,char *CID,char *CardName, unsigned long nScreenSize)
{
	...
	if (!Fid)
		return 0;
   db_fread2 (&setupdata,7,sizeof(setupdata),Fid,CL_FALSE);
	db_fclose (Fid);
   swap32 ((long *)&setupdata.UTMZone);
   swap32 ((long *)&setupdata.MaxDepthLayer);
	strncpy (MapPrefix,setupdata.MapPrefix_SufFile_dek,2);
	MapPrefix[2] = 0;
	...
	isFTCChip = cl_freadS32(Fid);
	if (isFTCChip == 9182012)
		isFTCChip = 1;
	else
		isFTCChip = 0;
	db_fclose (Fid);

   //MAXBUFFEREDIMAGETILES = (42 * nScreenSize) / 1024;
   //MAXBUFFEREDIMAGETILES += MAXBUFFEREDIMAGETILES % 2;
	LoadHBParms (PathToLKMData);
	LoadHBSymList (PathToLKMData);
	InitialImageTileSize = MAXBUFFEREDIMAGETILES;
	AllocateImageTileBuffer (MAXBUFFEREDIMAGETILES);
	return UTMZone;
}
...
void SetLKMImageMode (int mode)
{
	...
	{
		currentMode = mode;
		if (mode == IMAGE_MODE_NORTHUP)
			isize = InitialImageTileSize;
		else if (mode == IMAGE_MODE_COURSEUP)
			isize = InitialImageTileSize / 2;
		else
			isize = 0;
		ClearImageTileBuffer ();
		AllocateImageTileBuffer (isize);
	}
	return;
}
...
{
	LKMTrace ("LKMToHBClosed invoked.");
	InitReadBuffer(TRUE);
...
int GetObjectText (LPMAPPOINTOUTREC pPointOutRec,HANDLE *TextHandle)
{
  ...
  ClAssert( NULL != *TextHandle );
	...
	{
      if (!db_fread2 (&IndexRecordBlock,8,(TextFileHeader.NumPerIndexRec+1)*sizeof(GRIDINDEXREC),Fid,CL_TRUE))
			return 0;
  ...
  ClAssert( NULL != *TextHandle );
	pText = (char *)*TextHandle;
   if (!db_fread2 (pText, 9, nChar, Fid,CL_TRUE))
		nChar = 0;
	pText[nChar] = 0;
...
void DisplayTextChar (HANDLE HBImageHandle,char *chr, int nchar,int WorldX, int WorldY,int Rotation,int Size,CL_BOOL DepthPoint)
{
   ...
   if ((!FlipText ) ||
       (Rotation >= 0 && ( Rotation <= 900 || Rotation >= 2700)))
	...
	if (Level != CurSubFileLev)
	{
		...
		if (FidIndexIB != 0)
		{
			...
			for (i=0;i<nSubFiles;i++)
			{
			 ...
			 {
				 char msg[16];
				 LKMTrace ("Opens X file: ");
				 LKMTrace (ImageLibraryIndex);
				 sprintf (msg,"nSubfiles=%i",nSubFiles);
				 LKMTrace (msg);
			 }
	  }
	  else
	  {
		  LKMTrace ("Fails to open X file: ");
		  LKMTrace (ImageLibraryIndex);
	  }
              ...
              ClChartRenderCb pfCallBack, void *pCallBackData,LPINT pRc )
{
   ...
   LKMTrace ( (char *)__FUNCTION__);
	inc = (int) (*pScale * HBImageWidth / 2);
	HBImageBounds.xmn = CenterX - inc - 1;
	HBImageBounds.xmx = CenterX + inc + 1;
	inc = (int) (*pScale * HBImageHeight / 2);
	HBImageBounds.ymn = CenterY - inc - 1;
	HBImageBounds.ymx = CenterY + inc + 1;
...
#ifdef LKMTRACE	
   sprintf (LKMTRACESTRING,"Add tile:%d %d %c %d %d %d %d %d %d %d",pLKMTile->CellID,pLKMTile->iScale,pLKMTile->SubFileID,
												*NumScreenPixelsCovered, RowsOnScreen, ColsOnScreen,
												pLKMTile->destx,pLKMTile->desty,pLKMTile->destw,pLKMTile->desth);
   LKMTrace (LKMTRACESTRING);
   sprintf (LKMTRACESTRING,"Tile dimensions:%d %d %d %d",pLKMTile->xoff,pLKMTile->yoff,
								...
								ClChartRenderCb pfCallBack, void *pCallBackData )
{
	...
	HBImageBounds.xmn = CenterX - (int)(HBImageWidth/2 * Scale) - 1;
	HBImageBounds.xmx = CenterX + (int)(HBImageWidth/2 * Scale) + 1;
	HBImageBounds.ymn = CenterY - (int)(HBImageHeight/2 * Scale) - 1;
	HBImageBounds.ymx = CenterY + (int)(HBImageHeight/2 * Scale) + 1;
      ...
      if (LKMBeginEnumObjects (CenterX,CenterY,RangeInMeters,Scale,12,&enumHandle ))
		{
			...
			double	pixelSizeInSqFeet = (Scale * Scale) * SMSFT;
		 ...
		 MinPopulation = 8 * 8 * 8 * pixelSizeInSqFeet;
...
double GetDepthAtPoint (LPDPOINT Point, int nPickAperature)
{
   ...
   //RangeInMeters = Scale * nPickAperature + 1;
   Scale = AdjustToClosestScale (GET_DEPTH_SCALE);
   RangeInMeters = Scale;
   LKMBeginEnumObjectsText (pickPoint.x, pickPoint.y, RangeInMeters, Scale, &enumHandle );


   pEnum = (LPENUMSTRUCT)enumHandle;
              ...
              ClChartRenderCb pfCallBack, void *pCallBackData ,LPINT pRc)
{
...
#ifdef LKMTRACE	
   sprintf (LKMTRACESTRING,"Entering LakeMasterToHBirdImage_internal Scale:%f, XY:%d %d",*pScale,CenterX,CenterY);
   LKMTrace (LKMTRACESTRING);
#endif
	...
	for (iScale=1;iScale<NumGrids;iScale++)
   ...
   PixelsPerScreen = (HBImageWidth * HBImageHeight);
	do
	{
		...
		iScale--;
#ifdef LKMTRACE	
   sprintf (LKMTRACESTRING,"Pixels covered:%d %d",NumScreenPixelsCovered, PixelsPerScreen);
   LKMTrace (LKMTRACESTRING);
#endif
...
#ifdef LKMTRACE	
   sprintf (LKMTRACESTRING,"GetLKMImageInit ForceLevel:%d, SubFileId:%c",ForceLevel,SubFileID);
   LKMTrace (LKMTRACESTRING);
#endif
	if (ForceLevel >= 0)
		i = ForceLevel;
	else
	{
		for (i=0;i<NumGrids;i++)
      {
			if (Scale >= GridDefs[i].MetersPerPixel)
         {
            LKMTrace ("GetLKMImageInit -GoToHaveGrid");
				goto HaveGrid;
         }
      }
		i--;
...
#ifdef LKMTRACE
   sprintf (LKMTRACESTRING,"GetLKMImageInit - Opening:%s", ImageLibraryIndex);
	LKMTrace (LKMTRACESTRING);
#endif
   if (FidIndex != NULL)
   {
      LKMErrorTrace ("GetLKMImageInit - Image file already open");
	}
	...
	{
		nGridCol = 0;
      LKMErrorTrace ("GetLKMImageInit - Image file opend failed");
		return FALSE;
	}
#ifdef LKMTRACE
   sprintf (LKMTRACESTRING,"GetLKMImageInit - ImageOpened:%s", ImageLibraryIndex);
	LKMTrace (LKMTRACESTRING);
#endif
	if ((ioff = GetLakeOffset (FidIndex,CL_TRUE)) < 0)
	{
		nGridCol = 0;
      LKMErrorTrace ("GetLKMImageInit - Lake Offset is negative. Terminating LKMImage");
		GetLKMImageTerminate ();
		return FALSE;
	}
...
void SetWidePoint (LPBYTE image,CL_POINT pt,BYTE color,int iWidth,PLKMTILE pLKMTile)
{
	...
	if (pt.x >= 0 && pt.x < pLKMTile->width &&
		pt.y >= 0 && pt.y < pLKMTile->height)
   {
		image[iPixel] = color;
   }
  ...
  ClAssert( NULL != pLKMTile );
		...
		if (piPilot->lenTileCrossingPointArray > 0)
		{
			...
			if (PointInBound (piPilot->pTileCrossingPointArray[0],&pLKMTile->CellBounds64))
			{
				...
				SetWidePoint (pLKMTile->Image,nextPoint,pLKMTile->TrackColorPaletteIndex[0],cpBlockingSize,pLKMTile);
			}
...
void AllocateImageTileBuffer (int size)
{
   if (ImageTileBuffer == NULL)
   {
	MAXBUFFEREDIMAGETILES = size;
	if (MAXBUFFEREDIMAGETILES > 0)
	{

         if ( cl_chkmalloc (MAXBUFFEREDIMAGETILES * sizeof(IMAGETILEBUFFERELEMENT)))
         {
		...
		if (ImageTileBuffer == 0)
		{
         ...
         {
            memset (ImageTileBuffer,0,MAXBUFFEREDIMAGETILES * sizeof(IMAGETILEBUFFERELEMENT));
         }
      }
	}
	return;
...
void AddImageTileToBuffer (PLKMTILE pLKMTile)
{
   ...
   if ( (ImageTileBuffer != NULL) && (MAXBUFFEREDIMAGETILES > 0) )
   {
	for (i=0;i<MAXBUFFEREDIMAGETILES;i++)
	{
		pBufTile = (PLKMTILE)&ImageTileBuffer[i];
	...
	pBufTile = (PLKMTILE)&ImageTileBuffer[mini];
      memmove (pBufTile,pLKMTile,IMAGETILESIZE_ALIGNED);
...
PLKMTILE GetImageTileFromBuffer (int iScale,int CellID)
{
	...
	{
		pBufTile = (PLKMTILE)&ImageTileBuffer[i];
		if (pBufTile->SubFileID == currentSubFileID && pBufTile->iScale == iScale && pBufTile->CellID == CellID)
		{
         pLKMTile = (PLKMTILE)lm_malloc (IMAGETILESIZE_ALIGNED);
			ClAssert( NULL != pLKMTile );
         memmove (pLKMTile,pBufTile,IMAGETILESIZE_ALIGNED);
		 pLKMTile->Image = &pLKMTile->winBitmap[54+1024];
...
PLKMCTEXTTILE GetCTextTileFromBuffer (int CellID)
{
	...
	while (*pLen && i + sizeof(int) + *pLen < CTEXTBUFFERSIZE)
	{
		...
		if (pBufTile->CellID == CellID && pBufTile->SubFileID == currentSubFileID)
		{
...
{
	PLKMTILE pLKMTile;
   LPBYTE   pCompressdTile;
   int   tilelen, storedlen;
	static	int	nRead=0;
   ...
   if (FileHeader.NumPerIndexRec > 32)
	...
	{
      if (!db_fread2 (&IndexRecordBlock,10,(FileHeader.NumPerIndexRec+1)*sizeof(GRIDINDEXREC),FidIndex,CL_TRUE))
			return 0;
      if (FileHeader.NumPerIndexRec > 32)
	...
	do
	{
		if (db_fseek (FidIndex,loc,SEEK_SET))
			return 0;
      if (!db_fread2 (&ID_Length,11,8,FidIndex,CL_TRUE))
			return 0;
      swap32 ((long *)&ID_Length.ID);
      swap32 ((long *)&ID_Length.Length);
		ID = ID_Length.ID;
   ...
   pCompressdTile = (LPBYTE)lm_malloc (storedlen);
   ClAssert( NULL != pCompressdTile );
   if ( pCompressdTile )
   {
      if (!db_fread2 (pCompressdTile,12,storedlen,FidIndex,CL_TRUE))
      {
         cl_free (pCompressdTile);
         return 0;
      }
   }
   else
   ...
   tilelen = sizeof (LKMTILE)+FileHeader.TileWidth*FileHeader.TileHeight + 1024 + 54;
   tilelen += ( tilelen % 4 );
   pLKMTile = (PLKMTILE)lm_malloc (tilelen);
  ClAssert( NULL != pLKMTile );
   if ( !pLKMTile )
	{
      cl_free (pCompressdTile);
	...
	nRead += storedlen;
   DecompOK = DecompressTile (pLKMTile->winBitmap, tilelen, pCompressdTile, storedlen );
   cl_free (pCompressdTile);
   // if ( !DecompOK )
   // {
   //    attempt a re-read of the tile from disk
   // }
   if ( DecompOK )
	{
		pLKMTile->width = FileHeader.TileWidth;
		pLKMTile->height = FileHeader.TileHeight;
        memmove (pLKMTile->Palette,&pLKMTile->winBitmap[54],1024);
	    pLKMTile->Image = &pLKMTile->winBitmap[54+1024];
        //memmove (pLKMTile->Image,&pLKMTile->Image[54+1024],pLKMTile->width*pLKMTile->height);

		pLKMTile->SubFileID = currentSubFileID;
	...
	{
      WriteParamToErrorLog("Decompress Tile 3017 failed ", storedlen );
		*pRc = 1;
		cl_free (pLKMTile);
		pLKMTile = 0;
	}
...
PLKMTILE GetLKMImageTileFromScaleAndID (int iScale,int CellID,LPIPILOTSTRUCT piPilot,LPINT pRc,CL_BOOL UseBuffer)
{
   ...
   Top:
	...
	{
      if (!db_fread2 (&IndexRecordBlock2,10,(FileHeaderSI.NumPerIndexRec+1)*sizeof(GRIDINDEXREC),FidIndexSI,CL_FALSE))
	...
	{
		cl_fseek (FidIndexSI,loc,SEEK_SET);
      if (!db_fread2 (&ID_Length,11,8,FidIndexSI,CL_FALSE))
			goto NextFile;
      swap32 ((long *)&ID_Length.ID);
      swap32 ((long *)&ID_Length.Length);
		ID = ID_Length.ID;
   ...
   if ( pCompressdTile )
   {
      if (!db_fread2 (pCompressdTile,12,storedlen,FidIndexSI,CL_FALSE))
      {
         cl_free (pCompressdTile);
         goto NextFile;
      }
  ...
  ClAssert( NULL != pLKMTile );
   ...
   if ( DecompOK )
	...
	{
      WriteParamToErrorLog("Decompress Tile 3172 failed ", storedlen );
		*pRc = 1;
		cl_free (pLKMTile);
		pLKMTile = 0;
	}
...
CL_BOOL DecompressTile (LPBYTE pImage,int DecompressedLen, LPBYTE pCompressedImage, int compressedlen)
{
   ...
   pCompressedByteArray = (LPBYTE)lm_malloc (DecompressedLen);
   ClAssert( NULL != pCompressedByteArray );
   if ((pCompressedImage) && ( pImage ) && ( pCompressedByteArray ))
   {
      lbin = DecompressBinaryRecord (pCompressedByteArray,DecompressedLen,pCompressedImage,compressedlen);
      ClAssert( 0 != lbin );
      if (lbin )
   {
         lbin = DeCompressByteArray (pCompressedByteArray,lbin,pImage,DecompressedLen);
...
{   
   ...
   if (HaveLZOInit==FALSE)
   {
    ...
    HaveLZOInit = TRUE;	
      ...
      {
         WriteParamToErrorLog("Init LZO failed ", LZOerror );
      }
   }
   return HaveLZOInit;
}
...
int DecompressBinaryRecord (LPBYTE pDecompressedRec,int DecompressedLen,LPBYTE pCompressedRec,int CompressedLen)
{
   ...
   if ( InitLZO())
	{    
      DCLen = DecompressedLen;
      LZOerror = lzo1x_decompress_safe ( pCompressedRec, (lzo_uint)CompressedLen,
                                         pDecompressedRec,(lzo_uint *)&DCLen, NULL);
      if (LZOerror == 0)
      {
         rtn = DCLen;
	}
      ...
      {
         WriteParamToErrorLog("LZO Decompress failed ", LZOerror );
      }
   }
   return rtn;
}
...
int DeCompressByteArray (LPBYTE pMemCmp,int lMem,LPBYTE pDestMem,int DestLen)
{
   ...
   bBadData = FALSE;
   while ((lMem > 0) && ( !bBadData ))
	{
      plen2 = (signed char   *)pMemCmp++;
      len2 = *plen2;

		if (len2 > 0)
		{
			lNonRun = len2;
			if (( lDeCmp + lNonRun ) > DestLen )
         {
            WriteInformationToErrorLog("DecompressByteArray Overun  " );
            bBadData = TRUE;
				lDeCmp = 0;
         }
         else
         {
            memmove (pDestMem,pMemCmp,lNonRun);
            pDestMem += lNonRun;
			pMemCmp += lNonRun;
			lMem -= (lNonRun + 1);
			lDeCmp += lNonRun;
...
CL_BOOL LKMBeginEnumObjectsText (int WorldPtX,int WorldPtY,double RangeInMeters,double Scale,HANDLE *enumHandle )
{
	...
	HBImageBounds.xmn =  WorldPtX - inc - 1;
	HBImageBounds.xmx =  WorldPtX + inc + 1;
	HBImageBounds.ymn =  WorldPtY - inc - 1;
	HBImageBounds.ymx =  WorldPtY + inc + 1;
...
PLKMCTEXTTILE GetLKMCTextTile (LPCL_POINT pTilePoint)
{
   ...
   HaveTile:
	...
	{
      if (!db_fread2 (&IndexRecordBlock,15,(CTextFileHeader.NumPerIndexRec+1)*sizeof(GRIDINDEXREC),FidIndexCT,CL_TRUE))
			return 0;
		nreads++;
	...
	{
		db_fseek (FidIndexCT,loc,SEEK_SET);
      if (!db_fread2 (&ID_Length,16,8,FidIndexCT,CL_TRUE))
			return 0;
      swap32 ((long *)&ID_Length.ID);
      swap32 ((long *)&ID_Length.Length);
		ID = ID_Length.ID;
  ...
  ClAssert( NULL != pLKMTile );
   ...
   if (!db_fread2 (pLKMTile->TextDef,17,pLKMTile->Numrecs*sizeof(CTEXTREC),FidIndexCT,CL_TRUE))
	{
		cl_free (pLKMTile);
		return 0;
	}
...
CL_BOOL LKMEnumNextObject(HANDLE enumHandle, HANDLE *ObjectHandle  )	
{
   ...
   HaveTile:
	...
	{
      if (!db_fread2 (&IndexRecordBlock,18,(pEnumObject->ObjectFileHeader.NumPerIndexRec+1)*sizeof(GRIDINDEXREC),pEnumObject->Fid,CL_TRUE))
			return 0;
	...
	{
		db_fseek (pEnumObject->Fid,loc,SEEK_SET);
      if (!db_fread2 (&ID_Length,19,8,pEnumObject->Fid,CL_TRUE))
			return FALSE;
      swap32 ((long *)&ID_Length.ID);
      swap32 ((long *)&ID_Length.Length);
		ID = ID_Length.ID;
  ...
  ClAssert( NULL != pEnumObject->RecordHandle );
	...
	pLKMTile->Nextrec = 0;
	pLKMTile->CellBounds = CellBounds;
   if (!db_fread2 (pLKMTile->PointDef,20,pLKMTile->Numrecs*sizeof(MAPPOINTREC),pEnumObject->Fid,CL_TRUE))
	{
		cl_free (pEnumObject->RecordHandle);
		pEnumObject->RecordHandle = 0;
		return FALSE;
...
CL_BOOL LKMEnumNextName (HANDLE enumHandle, HANDLE *ObjectHandle  )	
{
   ...
   HaveTile:
	...
	for (i=0;i<pEnumObject->ObjectFileHeader.NumIndexLevs;i++)
	{
      if (!db_fread2 (&IndexRecordBlock,21,(pEnumObject->ObjectFileHeader.NumPerIndexRec+1)*sizeof(GRIDINDEXREC),pEnumObject->Fid,CL_TRUE))
			return FALSE;
      for (block=0; block<pEnumObject->ObjectFileHeader.NumPerIndexRec+1; block++)
      {
			swapGRIDINDEXREC(&(IndexRecordBlock[block]));
	...
	{
		db_fseek (pEnumObject->Fid,loc,SEEK_SET);
      if (!db_fread2 (&ID_Length,22,8,pEnumObject->Fid,CL_TRUE))
			return FALSE;
      swap32 ((long *)&ID_Length.ID);
      swap32 ((long *)&ID_Length.Length);
		ID = ID_Length.ID;
  ...
  ClAssert( NULL != pEnumObject->RecordHandle );
	...
	pLKMTile->Nextrec = 0;
	pLKMTile->CellBounds = CellBounds;
   if (!db_fread2 (pLKMTile->PointDef,23,pLKMTile->Numrecs*sizeof(CITYLAKENAMEREC),pEnumObject->Fid,CL_TRUE))
	{
		cl_free (pEnumObject->RecordHandle);
		pEnumObject->RecordHandle = 0;
		return FALSE;
...
CL_BOOL LKMGetObjectNameFromID (int ID,char *Name)
{
	...
	if (!Fid)
		return FALSE;
   db_fread2 (list,24, sizeof(list), Fid,CL_FALSE);
	cl_fclose (Fid);
...
static CL_BOOL cl_freadTEXTFILEHEADER( CTEXTFILEHEADER *header, CLFILE *pFid )
{  
   if (!db_fread2 (header,28,sizeof(CTEXTFILEHEADER),pFid,CL_TRUE))
	  return FALSE;
...
static CL_BOOL cl_freadOBJECTFILEHEADER( OBJECTFILEHEADER *header, CLFILE *pFid )
{  
   if (!db_fread2 (header,29,sizeof(OBJECTFILEHEADER),pFid,CL_TRUE))
	  return FALSE;
  swap32 (&header->GridID);
  swap32 (&header->NumPerIndexRec);
  swap32 (&header->NumIndexLevs);
...
static void swapCTEXTREC( CTEXTREC *rec)
{
   swap32 ((long *)&rec->CharNo_Chr);
   swap32 ((long *)&rec->x_y);
   swap32 ((long *)&rec->size_rot);
  return;
}
...
{
   swap32 ((long *)&rec->ref);
   swap32 ((long *)&rec->symno_hasextendedtext);
   swap32 ((long *)&rec->x_y);
}

void swapCITYLAKENAMEREC( CITYLAKENAMEREC *rec , CLFILE *pFID )
{
   swap32 ((long *)&rec->PopulationOrAcres);
   swap32 ((long *)&rec->type_nchar);
   swap32 ((long *)&rec->x_y);
}
int ProcessiPilotCommand (LPIPILOTSTRUCT piPilot,double Scale)
{
	...
	int	minStepsBetweenTrackPoints2;
	int stepsSinceLastTrackPoint=0;
	int stepsSinceLastTrackPoint2=0;
	CL_POINT pickPoint;
	DPOINT	offsetPoint;
   ...
   Top:
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
	minStepsBetweenTrackPoints2 = (int)(selectionTrackSpacing / Scale)/3;
#endif
	...
	switch (piPilot->iOpt)
	{
		...
		case 7:
		case 8:
			HBImageBounds.xmn = (int)(piPilot->pickPointD.x - saveImageWidth * Scale/2) - 1;
			HBImageBounds.ymn = (int)(piPilot->pickPointD.y - saveImageHeight * Scale/2) - 1;
			HBImageBounds.xmx = (int)(piPilot->pickPointD.x + saveImageWidth * Scale/2) + 1;
			HBImageBounds.ymx = (int)(piPilot->pickPointD.y + saveImageHeight * Scale/2) + 1;
		case 1:
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
			minStepsBetweenTrackPoints = (int)(selectionTrackSpacing / Scale)/3;
			if (piPilot->pSelectionTrack[0] != NULL)
			{
				piPilot->pSelectionTrack[0][0] = piPilot->pickPointD;
				piPilot->lenSelectionTrack[0] = 1;
				piPilot->pSelectionTrack[1][0] = piPilot->pickPointD;
				piPilot->lenSelectionTrack[1] = 1;
			}
#endif
		break;
		case 3:
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
			if (piPilot->pSelectedContour != NULL)
			{
				piPilot->pSelectedContour[0] = piPilot->pickPointD;
				piPilot->lenSelectedContour = 1;
			}
#endif
	...
	// off screen or until loop back to start point
	{
		...
		if (pLKMTile)
		{
         CL_POINT   nextPoint, pickedpt;
         CL_POINT   sideRing[2]={0};
			BYTE	wantPixelValue;
			int		iPixel;
			int		nsteps;
			int		breakRing=0;
			int		icolor=0;
			int		savecpBlockingSize = cpBlockingSize;
			...
			//if pick at corner reduce cpBlockSize temporarily
			if (nextPoint.x == 0 && nextPoint.y ==0 ||
				nextPoint.x == 0 && nextPoint.y ==255 ||
				nextPoint.x == 255 && nextPoint.y ==0 ||
				nextPoint.x == 255 && nextPoint.y ==255)
				cpBlockingSize = 4;
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
				if (piPilot->iOpt == 1 && piPilot->pSelectionTrack[0] != NULL && piPilot->lenSelectionTrack[0] < maxSelectionTrack && stepsSinceLastTrackPoint++ > minStepsBetweenTrackPoints)
				{
					DPOINT wpoint = TilePointToWorldPoint2 (nextPoint.x,nextPoint.y,pLKMTile);

					if (distpd (&wpoint,&piPilot->pSelectionTrack[0][piPilot->lenSelectionTrack[0]-1]) >= selectionTrackSpacing)
					{
						piPilot->pSelectionTrack[0][piPilot->lenSelectionTrack[0]++] = wpoint;
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
				if (piPilot->iOpt == 3 && piPilot->lenSelectedContour < maxSelectedContour && stepsSinceLastTrackPoint2++ > minStepsBetweenTrackPoints2)
				{
					DPOINT wpoint = TilePointToWorldPoint2 (nextPoint.x,nextPoint.y,pLKMTile);

                     // Bug Fix
                     // if (distpd (&wpoint,&piPilot->pSelectedContour[piPilot->lenSelectedContour]) >= selectionTrackSpacing)
               if (distpd (&wpoint,&piPilot->pSelectedContour[piPilot->lenSelectedContour-1]) >= selectionTrackSpacing)
					{
						piPilot->pSelectedContour[piPilot->lenSelectedContour++] = wpoint;
						stepsSinceLastTrackPoint2 = 0;
					}
				}
#endif
...
// Track the second direction until off screen			
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
				if (piPilot->iOpt == 1 && piPilot->pSelectionTrack[1] != NULL && piPilot->lenSelectionTrack[1] < maxSelectionTrack && stepsSinceLastTrackPoint++ > minStepsBetweenTrackPoints)
				{
					DPOINT wpoint = TilePointToWorldPoint2 (nextPoint.x,nextPoint.y,pLKMTile);

					if (distpd (&wpoint,&piPilot->pSelectionTrack[1][piPilot->lenSelectionTrack[1]-1]) >= selectionTrackSpacing)
					{
						piPilot->pSelectionTrack[1][piPilot->lenSelectionTrack[1]++] = wpoint;
						stepsSinceLastTrackPoint = 0;
					}
				}
#endif
			...
			if (sideRing[1].x && piPilot->lenTileCrossingPointArray < piPilot->maxTileCrossingPointArray)
               ...
               // Bug Fix
         if ( breakRing && piPilot->lenTileCrossingPointArray < piPilot->maxTileCrossingPointArray )
         {          
            piPilot->pTileCrossingPointArray[piPilot->lenTileCrossingPointArray++] = piPilot->pTileCrossingPointArray[0];
         }
            
			piPilot->iError = 0;
			cpBlockingSize = savecpBlockingSize;
		}
		if (piPilot->iOpt > 4)
		{
			 double dist1, dist2;
      ...
      depthRight = GetDepthAtPoint (&offsetPoint, piPilot->pickAperature);
		offsetPoint = dnewpt (&piPilot->directionPoint[0],az-HALFPI,-2*MAX_CONTOURTEXT_SCALE);
      depthLeft = GetDepthAtPoint (&offsetPoint, piPilot->pickAperature);
...
#ifdef LKMTRACE	
   sprintf (LKMTRACESTRING,"Enter LakeMasterToHBirdImage:%d %d %d %d %f",HBImageWidth,HBImageHeight,CenterX,CenterY, *pScale);
   LKMTrace (LKMTRACESTRING);
#endif
...
//	ipAddress->pickAperature = ININT(2 * pickOffset / scale);
	ipAddress->pickAperature = (int) (pickOffset*2);
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
	if (maxSelectionTrack > 0)
	{
		ipAddress->pSelectionTrack[0] = lm_malloc (maxSelectionTrack * sizeof(DPOINT));
		ipAddress->pSelectionTrack[1] = lm_malloc (maxSelectionTrack * sizeof(DPOINT));
	}
	if (maxSelectedContour > 0)
	{
		ipAddress->pSelectedContour = lm_malloc (maxSelectedContour * sizeof(DPOINT));
		ipAddress->lenSelectedContour = 0;
	}
#endif
#ifdef LKMTRACE
	...
	LKMTrace (LKMTRACESTRING);
#endif
   DepthOffset_Save = DepthOffset; 
   // Temporarily reset the DepthOffset to get the actual contour depth value
   DepthOffset = 0;   
	rtn = LakeMasterToHBirdImage_internal (NULL,2048,2048,ININT(pickX),ININT(pickY),&scale,
											...
											FALSE,ipAddress,
											NULL,NULL,&st);
   // Restore DepthOffset value                              
   DepthOffset = DepthOffset_Save;
   //strncpy (pickText,ipAddress->pickText,31);
   ipAddress->pickText[31] = 0;
#ifdef LKMTRACE
   ...
   // contour exists. Apply the water depth offset and see whether the contour still exists
   nContourDepth = atoi( ipAddress->pickText );
   if (nContourDepth+DepthOffset < 0 )
   {
      // no contour after adjusting the water depth offset.
      LKMiPilotClear (ipAddress);
      return NULL;
   }
   *PickDepthInFt = nContourDepth;
   sprintf(pickText, "%d feet", nContourDepth+DepthOffset);
...
{
   LPIPILOTSTRUCT piPilotNew;

   LKMTrace ( (char *)__FUNCTION__);

   piPilotNew = lm_malloc (sizeof(IPILOTSTRUCT));
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
	if (piPilotNew->pSelectedContour != NULL)
	{
		if (!piPilotNew->lenSelectedContour)
			piPilotNew->pSelectedContour = lm_malloc (maxSelectedContour * sizeof(DPOINT));
		else
			piPilotNew->pSelectedContour = lm_malloc (piPilotNew->lenSelectedContour * sizeof(DPOINT));
		ClAssert( NULL != piPilotNew->pSelectedContour );
		memcpy (piPilotNew->pSelectedContour,ipAddress->pSelectedContour,piPilotNew->lenSelectedContour * sizeof(DPOINT));
	}
	if (piPilotNew->pSelectionTrack[0] != NULL)
	{
		if (!piPilotNew->lenSelectionTrack[0])
			piPilotNew->pSelectionTrack[0] = lm_malloc (maxSelectionTrack * sizeof(DPOINT));
		else
			piPilotNew->pSelectionTrack[0] = lm_malloc (piPilotNew->lenSelectionTrack[0] * sizeof(DPOINT));
		ClAssert( NULL != piPilotNew->pSelectionTrack[0] );
		memcpy (piPilotNew->pSelectionTrack[0],ipAddress->pSelectionTrack[0],piPilotNew->lenSelectionTrack[0] * sizeof(DPOINT));
	}
	if (piPilotNew->pSelectionTrack[1] != NULL)
	{
		if (!piPilotNew->lenSelectionTrack[1])
			piPilotNew->pSelectionTrack[1] = lm_malloc (maxSelectionTrack * sizeof(DPOINT));
		else
			piPilotNew->pSelectionTrack[1] = lm_malloc (piPilotNew->lenSelectionTrack[1] * sizeof(DPOINT));
		ClAssert( NULL != piPilotNew->pSelectionTrack[1] );
		memcpy (piPilotNew->pSelectionTrack[1],ipAddress->pSelectionTrack[1],piPilotNew->lenSelectionTrack[1] * sizeof(DPOINT));
	}
#endif
	piPilotNew->pTrackArrayOut = NULL;
	piPilotNew->pTrackArrayDepth = NULL;
   piPilotNew->pTrackArrayDepthOut = NULL;
	piPilotNew->lenTrackArrayOut = 0;
	piPilotNew->lenTrackArrayTemp = 0;
#if USE_CONTOUR_SELECTION_TRACK_POINTS
	piPilotNew->pSelectedContourOut = NULL;
	piPilotNew->pSelectionTrackOut[0] = NULL;
	piPilotNew->pSelectionTrackOut[1] = NULL;
#endif
									   ...
									   int knotRemovalRange, int smoothOpt,CL_BOOL wantDepths)
{
	...
	int	rtn;
	double	scale;
   int      DepthOffset_Save = DepthOffset;
#ifdef LKMTRACE
   sprintf (LKMTRACESTRING,"LKMiPilotRequestTrack:%i opt=%i x=%f y=%f off=%f kn=%i sm=%i dep=%i",
                            (int)ipAddress,trackOption,x,y,offsetInMeters,
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
		if (piPilot->pSelectedContourOut != NULL)
			cl_free (piPilot->pSelectedContourOut);
		piPilot->pSelectedContourOut = 0;
		if (piPilot->pSelectedContour != NULL)
			cl_free (piPilot->pSelectedContour);
      if (maxSelectedContour > 0)
      {
		piPilot->pSelectedContour = lm_malloc (maxSelectedContour * sizeof(DPOINT));
		piPilot->lenSelectedContour = 1;
		piPilot->pSelectedContour->x = x;
		piPilot->pSelectedContour->y = y;
      }
#endif
	...
	else if (trackOption == 5)
	{
      ...
      if ( piPilot->pTileCrossingPointArray != NULL && piPilotOld->maxTileCrossingPointArray > 0 )
      {
       memmove(piPilot->pTileCrossingPointArray,piPilotOld->pTileCrossingPointArray,piPilotOld->maxTileCrossingPointArray * sizeof(CL_POINT));
      }
      if (piPilotOld->pTrackArray != NULL)
      {
         piPilot->pTrackArray = lm_malloc (piPilotOld->lenTrackArray * sizeof(DPOINT));
         ClAssert( NULL != piPilot->pTrackArray );
         memcpy (piPilot->pTrackArray,piPilotOld->pTrackArray,piPilotOld->lenTrackArray * sizeof(DPOINT));
      }
      else
      {
        // log error 
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
		if (piPilotOld->pSelectedContourOut != NULL && piPilotOld->lenSelectedContour > 0)
		{
			piPilot->pSelectedContourOut = lm_malloc (piPilot->lenSelectedContour * sizeof(CL_POINT));
         memmove (piPilot->pSelectedContourOut,piPilotOld->pSelectedContourOut,piPilotOld->lenSelectedContour * sizeof(CL_POINT));
		}
#endif
	}
	else if (trackOption == 6 || trackOption == 7)
	{
		...
		atPoint.x = x;
		atPoint.y = y;
		ConvertLatLontoUTM (&atPoint,UTMZone,cActualZone);
      ...
      if (maxSelectedContour > 0)
      {
		piPilot->pSelectedContour = lm_malloc (maxSelectedContour * sizeof(DPOINT));
		piPilot->lenSelectedContour = 1;
		piPilot->pSelectedContour[0] = nearPoint;
      }
#endif
   ...
   LKMTrace ( "ReSampleToOriginalSpacing" );
	...
	if (wantDepths)
	{
      ...
      {
         piPilot->pTrackArrayDepth[i] = (int)(GetDepthAtPoint (&piPilot->pTrackArrayTemp[i], piPilot->pickAperature)*10);
      }
      DepthOffset = DepthOffset_Save;
      
      piPilot->pTrackArrayDepthOut = lm_malloc (piPilot->lenTrackArrayOut * sizeof(int));
      ClAssert( NULL != piPilot->pTrackArrayDepthOut );
	...
	for (i=0;i<piPilot->lenTrackArrayOut;i++)
	{
		...
		piPilot->pTrackArrayOut[i].x = (int) (piPilot->pTrackArrayTemp[i].x * 1000000);
		piPilot->pTrackArrayOut[i].y = (int) (piPilot->pTrackArrayTemp[i].y * 1000000);
	}
...
#if USE_CONTOUR_SELECTION_TRACK_POINTS
	if (ipAddress->pSelectionTrack[0] != NULL)
		cl_free (ipAddress->pSelectionTrack[0]);
	ipAddress->pSelectionTrack[0] = 0;
	if (ipAddress->pSelectionTrack[1] != NULL)
		cl_free (ipAddress->pSelectionTrack[1]);
	ipAddress->pSelectionTrack[1] = 0;
	if (ipAddress->pSelectionTrackOut[0] != NULL)
		cl_free (ipAddress->pSelectionTrackOut[0]);
	ipAddress->pSelectionTrackOut[0] = 0;
	if (ipAddress->pSelectionTrackOut[1] != NULL)
		cl_free (ipAddress->pSelectionTrackOut[1]);
	ipAddress->pSelectionTrackOut[1] = 0;
	if (ipAddress->pSelectedContour != NULL)
		cl_free (ipAddress->pSelectedContour);
	ipAddress->pSelectedContour = 0;
	if (ipAddress->pSelectedContourOut != NULL)
		cl_free (ipAddress->pSelectedContourOut);
	ipAddress->pSelectedContourOut = 0;
#endif
	if (ipAddress->pTrackArrayDepth != NULL)
		cl_free (ipAddress->pTrackArrayDepth);
	
   if (ipAddress->pTrackArrayDepthOut != NULL)
      cl_free (ipAddress->pTrackArrayDepthOut);
...
/******************************************************************************
 *
 *    Function: GetFTCTrackDepths
 *
 *    Args:     ipAddress - iPilotStruct address
 *              pNumPoints [out] - pointer to int to return number of track depth points
 *    Return:   Pointer to the track depth array
 *
 *    Purpose:  Applies water depth offset to actual track depth array.
 *
 *    Notes:
 *
 ******************************************************************************/
int	*GetFTCTrackDepths (LPVOID ipAddress,int *pNumPoints)
{
	...
	*pNumPoints = piPilot->lenTrackArrayOut;
   ClAssert( NULL != piPilot->pTrackArrayDepthOut );
   // Apply the depth offset to orignal track depth array before copying to TrackArrayDepthOut
   for (i=0;i<piPilot->lenTrackArrayOut;i++)
   {
     piPilot->pTrackArrayDepthOut[i] = CL_MAX(0, (piPilot->pTrackArrayDepth[i]+(DepthOffset*10)));
   }
   return(int *)piPilot->pTrackArrayDepthOut;
}
...
{
#if CHECK_FTC_COMPATIBILITY
   return isFTCChip;
#else
   return 1;
#endif // CHECK_FTC_COMPATIBILITY
}
...
#ifdef LKMTRACE
	sprintf (LKMTRACESTRING,"GetFTCSelectionTrack:%i nai=%i", (int)ipAddress,numActiveiPilots);
	LKMTrace (LKMTRACESTRING);
#endif
	*pNumPoints = 0;
	if (piPilot == NULL || iWhichTrack < 0 || iWhichTrack > 1 || piPilot->lenSelectionTrack[iWhichTrack] == 0)
		return NULL;

	*pNumPoints = piPilot->lenSelectionTrack[iWhichTrack];
#ifdef LKMTRACE
	sprintf (LKMTRACESTRING,"GetFTCSelectionTrack:%i np=%i", (int)ipAddress,*pNumPoints);
	LKMTrace (LKMTRACESTRING);
#endif
	if (piPilot->pSelectionTrackOut[iWhichTrack] == NULL)
	{
		piPilot->pSelectionTrackOut[iWhichTrack] = lm_malloc (piPilot->lenSelectionTrack[iWhichTrack] * sizeof(CL_POINT));
		ClAssert( NULL != piPilot->pSelectionTrackOut[iWhichTrack] );
		for (i=0;i<piPilot->lenSelectionTrack[iWhichTrack];i++)
		{
			ConvertUTMtoLatLon (&piPilot->pSelectionTrack[iWhichTrack][i],UTMZone);
			piPilot->pSelectionTrackOut[iWhichTrack][i].x = (int) (piPilot->pSelectionTrack[iWhichTrack][i].x * 1000000);
			piPilot->pSelectionTrackOut[iWhichTrack][i].y = (int) (piPilot->pSelectionTrack[iWhichTrack][i].y * 1000000);
		}
		cl_free (piPilot->pSelectionTrack[iWhichTrack]);
		piPilot->pSelectionTrack[iWhichTrack] = NULL;
	}
   return(LPFTC_POINT)piPilot->pSelectionTrackOut[iWhichTrack];
}
...
#ifdef LKMTRACE
	sprintf (LKMTRACESTRING,"GetFTCSelectedContourTrack:%i nai=%i", (int)ipAddress,numActiveiPilots);
	LKMTrace (LKMTRACESTRING);
#endif
	*pNumPoints = 0;
	if (piPilot == NULL || piPilot->lenSelectedContour <= 0)
		return NULL;

	*pNumPoints = piPilot->lenSelectedContour;
#ifdef LKMTRACE
	sprintf (LKMTRACESTRING,"GetFTCSelectedContourTrack:%i np=%i", (int)ipAddress,*pNumPoints);
	LKMTrace (LKMTRACESTRING);
#endif
	if (piPilot->pSelectedContourOut == NULL)
	{
		piPilot->pSelectedContourOut = lm_malloc (piPilot->lenSelectedContour * sizeof(CL_POINT));
		ClAssert( NULL != piPilot->pSelectedContourOut );
		for (i=0;i<piPilot->lenSelectedContour;i++)
		{
			ConvertUTMtoLatLon (&piPilot->pSelectedContour[i],UTMZone);
			piPilot->pSelectedContourOut[i].x = (int) (piPilot->pSelectedContour[i].x * 1000000);
			piPilot->pSelectedContourOut[i].y = (int) (piPilot->pSelectedContour[i].y * 1000000);
		}
		cl_free (piPilot->pSelectedContour);
		piPilot->pSelectedContour = NULL;
	}
   return(LPFTC_POINT)piPilot->pSelectedContourOut;
}
...
void ClearFTCSelectionContours (LPVOID ipAddress)
{
	...
	if (piPilot->pSelectionTrack[0] != NULL)
		cl_free (piPilot->pSelectionTrack[0]);
	if (piPilot->pSelectionTrack[1] != NULL)
		cl_free (piPilot->pSelectionTrack[1]);
	piPilot->pSelectionTrack[0] = 0;
	piPilot->pSelectionTrack[1] = 0;
	piPilot->lenSelectionTrack[0] = 0;
	piPilot->lenSelectionTrack[1] = 0;
	if (piPilot->pSelectionTrackOut[0] != NULL)
		cl_free (piPilot->pSelectionTrackOut[0]);
	if (piPilot->pSelectionTrackOut[1] != NULL)
		cl_free (piPilot->pSelectionTrackOut[1]);
	piPilot->pSelectionTrackOut[0] = 0;
	piPilot->pSelectionTrackOut[1] = 0;
	return;
}

void SetFTCSelectSpacing (double trackPointSpacing)
{
	selectionTrackSpacing = trackPointSpacing;
	return;
}

void SetFTCMaxSelectionTrack (int maxSelTrack)
{
	maxSelectionTrack = maxSelTrack;
	return;
}

void SetFTCMaxSelectedContour (int maxSelTrack)
{
	maxSelectedContour = maxSelTrack;
	return;
}
#endif
