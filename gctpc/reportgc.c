...
static char err_file[256],str[256];
...
void radius(A)
  double A;
      {
      if (terminal_p)
         sprintf(str,"   Radius of Sphere:     %lf meters\n",A); 
      if (file_p)
	 {
         fptr_p = (FILE *)fopen(parm_file,"a");
...
void radius2(A,B)
  double A,B;
      ...
      if (terminal_p)
         {
         sprintf(str,"   Semi-Major Axis of Ellipsoid:     %lf meters\n",A);
         sprintf(str,"   Semi-Minor Axis of Ellipsoid:     %lf meters\n",B);
         }
      if (file_p)
         {
         fptr_p = (FILE *)fopen(parm_file,"a");
...
void cenlon(A)
  double A;
   { 
   if (terminal_p)
       sprintf(str,"   Longitude of Center:     %lf degrees\n",A*R2D);
   if (file_p)
       {
       fptr_p = (FILE *)fopen(parm_file,"a");
...
void cenlonmer(A)
  double A;
   { 
   if (terminal_p)
     sprintf(str,"   Longitude of Central Meridian:     %lf degrees\n",A*R2D);
   if (file_p)
     {
     fptr_p = (FILE *)fopen(parm_file,"a");
...
void cenlat(A)
  double A;
   {
   if (terminal_p)
      sprintf(str,"   Latitude  of Center:     %lf degrees\n",A*R2D);
   if (file_p)
      {
      fptr_p = (FILE *)fopen(parm_file,"a");
...
void origin(A)
  double A;
   {
   if (terminal_p)
      sprintf(str,"   Latitude of Origin:     %lf degrees\n",A*R2D);
   if (file_p)
      {
      fptr_p = (FILE *)fopen(parm_file,"a");
...
void stanparl(A,B)
  double A,B;
   ...
   if (terminal_p)
      {
      sprintf(str,"   1st Standard Parallel:     %lf degrees\n",A*R2D);
      sprintf(str,"   2nd Standard Parallel:     %lf degrees\n",B*R2D);
      }
   if (file_p)
      {
      fptr_p = (FILE *)fopen(parm_file,"a");
...
void stparl1(A)
  double A;
   ...
   if (terminal_p)
      {
      sprintf(str,"   Standard Parallel:     %lf degrees\n",A*R2D);
      }
   if (file_p)
      {
      fptr_p = (FILE *)fopen(parm_file,"a");
...
void offsetp(A,B)
  double A,B;
   ...
   if (terminal_p)
      {
      sprintf(str,"   False Easting:      %lf meters \n",A);
      sprintf(str,"   False Northing:     %lf meters \n",B);
      }
   if (file_p)
      {
      fptr_p = (FILE *)fopen(parm_file,"a");
...
void p_error(what, where) 
   ...
   {
//      sprintf(str,"[%s] %s\n",where,what);
      GSSiMsgBox (GetFocus(),what,where,MB_ICONEXCLAMATION,0); 
   }
   if (file_e)
      {
