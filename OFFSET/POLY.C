...
//    the derived area.      
    ...
    { 
       ...
       // now I create an area structure that emulates Ltable
          file[10].heap_ptr = GSSiGlobAlloc(GAIDNO 283, GHND, sizeof (LArea) ); 
          file[10].mem_ptr  = GlobalLock(file[10].heap_ptr);
          lpLA  = (lpLArea) file[10].mem_ptr; 
          file[11].heap_ptr = GSSiGlobAlloc(GAIDNO 284, GHND, sizeof (LINE)* (lpPoly1->i_count + 22) ); 
...
{  int i;
       ...
       {  //release any old grips on offset memory
          ...
          file[1].heap_ptr = GSSiGlobAlloc(GAIDNO 285, GHND, sizeof(LINE)*( MaxLineBase)); 
          ...
          file[2].heap_ptr = GSSiGlobAlloc(GAIDNO 286, GHND, sizeof (PolyCom1) ); 
          ...
          file[3].heap_ptr = GSSiGlobAlloc(GAIDNO 287, GHND, (long)sizeof (PolyCom2) *(long)MaxPoly2Base ); 
          file[3].mem_ptr  = GlobalLock(file[3].heap_ptr); 
          lpPoly2 = (lpPolyCom2) file[3].mem_ptr;
          lpPoly2Base = lpPoly2;
          MaxWorkBase = InArea->NumPts*3;
          file[4].heap_ptr = GSSiGlobAlloc(GAIDNO 288, GHND, (long)sizeof (WorkSpace)* (long)(MaxWorkBase) ); 
          file[4].mem_ptr  = GlobalLock(file[4].heap_ptr); 
          lpWork = (lpWS) file[4].mem_ptr; 
          lpWorkBase = lpWork;
          MaxNodeArrBase = InArea->NumPts * 3;
          file[5].heap_ptr = GSSiGlobAlloc(GAIDNO 289, GHND, sizeof (NodeArray)* (MaxNodeArrBase) ); 
          ...
          file[6].heap_ptr = GSSiGlobAlloc(GAIDNO 290, GHND, sizeof (gen_info)* MaxNumAreas ); 
          file[6].mem_ptr  = GlobalLock(file[6].heap_ptr); 
          lpGInfo = (lpGenInfo) file[6].mem_ptr;
          lpGInfoBase = lpGInfo;
          
          MaxAllSides = InArea->NumPts*6;
          file[7].heap_ptr = GSSiGlobAlloc(GAIDNO 291, GHND, sizeof (AllSides)* (MaxAllSides) ); 
          file[7].mem_ptr  = GlobalLock(file[7].heap_ptr); 
          lpAllS = (lpAS) file[7].mem_ptr;
          lpAllSBase = lpAllS;  
          MaxNewBase = InArea->NumPts * 32;
          file[8].heap_ptr = GSSiGlobAlloc(GAIDNO 292, GHND, sizeof (TempArea)* (MaxNewBase) ); 
