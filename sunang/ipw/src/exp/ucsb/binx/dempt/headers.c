...
headers()
{
  ...
  */
	if (parm.islqh && i_lqhpp != NULL) {
		...
		if (parm.units != NULL &&
		    (lqh_units(o_lqhpp[0]) = hstrdup(parm.units,
					     LQH_HNAME, 0)) == NULL) {
...
#ifndef	lint
static char     rcsid[] = "$Header: /home/ipw/src/bin/dempt/RCS/headers.c,v 1.4 89/11/17 14:22:05 frew Exp $";
