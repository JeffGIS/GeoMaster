...
void
radtbls(fdi, elev, mu0, tau, omega, gfact, R0, S0, bval, dval)
	...
	fpixel_t       *dval;		/* min, max diffuse rad (ret)	 */
{
  ...
  */
	press = HYSTAT(SEA_LEVEL, STD_TEMP, LAPSE,
		       elev * 1.e-3, GRAV, MOL_AIR);
 /*
  ...
  */
	for (j = 0; j < nvals; ++j) {
		zpress = HYSTAT(SEA_LEVEL, STD_TEMP, LAPSE,
				zp[j] * 1.e-3, GRAV, MOL_AIR);
...
#ifndef	lint
static char     rcsid[] = "$Header: radtbls.c,v 1.1 88/04/03 12:33:17 dozier Exp $";
