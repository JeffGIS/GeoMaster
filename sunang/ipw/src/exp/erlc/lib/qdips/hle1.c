...
**
** NAME
**	hle1 -- sensible & latent heat from data at 1 height
**
** SYNOPSIS
**	int hle1(press, ta, ts, za, ea, es, zq, u, zu, z0, h, le, e)
**	double press;
**	double ta;
**	double ts;
**	double za;
...
**
** DESCRIPTION
**	hle1 computes sensible and latent heat flux and mass flux given
...
**
**	Reference:  Brutsaert, W., Evaporation into the Atmosphere, 299
**		pp., D. Reidel, Dordrecht, 1982 (chapters 4, 5, and 9).
**	(Equation numbers refer to this book.)
** 
** RESTRICTIONS
**
** RETURN VALUE
...
** 
** HISTORY
**	June, 1987: Written as a Qdips program by J. Dozier, CRSEO, UCSB;
**	October, 1990: Translated into IPW by K. Longley, OSU, ERLC;
**
** FUTURE DIRECTIONS
** 
** BUGS
...
DEFUN( psi, (zeta, code),
	double	zeta		/* z/lo				*/
   AND  int	code)		/* which psi function? (see above) */
{
...
int
DEFUN( hle1, (press, ta, ts, za, ea, es, zq, u, zu, z0, h, le, e),

/* input variables */
	double	press	/* air pressure (Pa)			*/
   AND  double	ta	/* air temperature (K) at height za	*/
   AND  double	ts	/* surface temperature (K)		*/
   AND  double	za	/* height of air temp measurement (m)	*/
   AND  double	ea	/* vapor pressure (Pa) at height zq	*/
   AND  double	es	/* vapor pressure (Pa) at surface	*/
   AND  double	zq	/* height of spec hum measurement (m)	*/
   AND  double	u	/* wind speed (m/s) at height zu	*/
   AND  double	zu	/* height of wind speed measurement (m)	*/
   AND  double	z0	/* roughness length (m)			*/

/* output variables */
   AND  double *h	/* sens heat flux (+ to surf) (W/m^2)	*/
   AND  double *le	/* latent heat flux (+ to surf) (W/m^2)	*/
   AND  double *e)	/* mass flux (+ to surf) (kg/m^2/s)	*/
{
	double	ah = AH;
	double	av = AV;
	double	cp = CP_AIR;
