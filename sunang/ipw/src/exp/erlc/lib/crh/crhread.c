...
#ifndef lint
static char *SCCSid=	"SCCS version: @(#)   crhread.c   1.4   10/19/90";
#endif
...
**
** APPLICATION USAGE
**	crhread is called by application programs to ingest CRH headers.
**
** FUTURE DIRECTIONS
**
** BUGS
   	...
   	/* ingest records */
		...
		/* insist that first record is CRH_NCLASS key */
   		...
   		/* match key to header field, ingest value */
				...
				nclass++;
			} else if (streq(key, CRH_ANNOT) && (crhp->annot =
			   hstrdup(value, CRH_HNAME, band)) == NULL) {
				return (NULL);
			} else if (streq(key, CRH_UNITS) && (crhp->units =
			   hstrdup(value, CRH_HNAME, band)) == NULL) {
				return (NULL);
