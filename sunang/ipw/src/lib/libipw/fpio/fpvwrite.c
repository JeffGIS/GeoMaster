...
**
** GLOBALS ACCESSED
**	_piocb[fd]	floating-point I/O control block for file descriptor
**			fd (indirectly through _pioinit)
**
** ERRORS
**
** WARNINGS
**
** APPLICATION USAGE
**	fpvwrite writes floating-point values as IPW image pixels
**
** FUTURE DIRECTIONS
**
** BUGS
	...
	int             npixv;		/* # pixel vectors to write	 */
{
	...
	p = _fpioinit(fd, FPIO_WRITE, npixv);
	if (p == NULL) {
		return (ERROR);
	}
...
/*
				if (ABS(*bufp - p->map[band][*pixp]) > 0.5) {
				  fprintf(stderr, "binary search warning: %d (%d) %f -> %f\n", *pixp, x, *bufp, p->map[band][*pixp]);
				}
*/
...
#ifndef	lint
static char     rcsid[] = "$Header: /local/share/pkg/ipw/src/lib/libipw/fpio/RCS/fpvwrite.c,v 1.9 90/11/11 17:15:14 frew Exp $";
