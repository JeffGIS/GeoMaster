...
#ifndef lint
static char *SCCSid=	"SCCS version: @(#)   itrbxfr.c   4.5   10/19/90";
#endif
	...
	double	z;		/* upper height (m)			*/
{
	...
	double	h_min;		/* min h in output image		*/
	double	h_max;		/* max h in output image		*/
	double	le_min;		/* min le in output image		*/
	double	le_max;		/* max le in output image		*/
	double	mm_min;		/* min mm in output image		*/
	double	mm_max;		/* max mm in output image		*/
	...
	for (line=0; line < nlines; line++) {
		...
		for (samp=0; samp < nsamps; samp++) {
			...
			if (get_sample (fdi, inbuf, mbuf, samp, &index, vector,
				...
				pa = HYSTAT (SEA_LEVEL, TB, (STDLR * 1000.0),
				     (elev / 1000.0), GRAV, MOL_AIR);
