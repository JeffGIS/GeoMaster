 ...
 * Dale P. Smith and wmSun by Mike Henderson. Additional patches for
 * gkrellm 2.0 and Windows support from Bill Nalen.
 *
 * $Id: CalcEphem.c,v 1.3 2002/11/13 11:12:19 nwalsh Exp $
 *
 */
#ifdef HAVE_CONFIG_H
#include <config.h>
...
gdouble kepler(gdouble M, gdouble e)
{
	gint n = 0;
...
gint DayofYear(gint year, gint month, gint day)
{
	gdouble jd();
...
gint DayofWeek(gint year, gint month, gint day, gchar dowstr[])
{
...
gdouble angle360(gdouble angle)
{
	gint n;
...
void SunRise(int year, int month, int day, gdouble UT, CTrans *c, int debug) {
...
void sunclock_CalcEphem(glong date, gdouble UT, CTrans * c, int debug)
{
	gint year, month, day;
	...
	if (debug) {
	  printf("gkrellsun ephem debug: %d\n", debug);
	}
	...
	SunRise(year, month, day, UT, c, debug);
}
