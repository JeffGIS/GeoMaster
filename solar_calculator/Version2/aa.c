...
int GSSikinit (double *lon,double *lat,double *hgt);
...
int GetAltitudeAndAzOfSun (LPDPOINT pLatLon,double Height,time_t systime,double *pAltitude,double *pAz)
{
	int rtn=0;
	struct tm tms;
extern double azimuth, elevation, refracted_elevation;

	GSSikinit (&pLatLon->x,&pLatLon->y,&Height);

	tms = *gmtime (&systime);

	JD = caltoj(tms.tm_year + 1900,tms.tm_mon+1,(double)tms.tm_mday );
	JD += (3600.0*tms.tm_hour + 60.0*tms.tm_min + tms.tm_sec)/86400.0;
	update();
	elobject = 0;
...
//	iter_trnsit( dosun );
	...
	*pAltitude = refracted_elevation;//elevation;
	*pAz = azimuth;
	return rtn;
}