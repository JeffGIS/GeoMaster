...
#define GSSI_GMDOCCOMPLETE 0x5120
...
long SearchFilesInDir2(LPSTR CurDir, LPSTR Ext, HFILE OutFile, LPLONG TotFiles, LPSTR WildCard, int Lev, BOOL WantSub, BOOL fileNameOnly, int maxFiles);
...
int main_shptest(int argc, char** argv);
...
INT64	GetDriveFreeSpace (LPSTR Drive);
...
MNMXCORD3D atobounds3D(LPSTR Value, LPBOOL pErr);
...
HDIB32 LoadDIB32(LPSTR lpFileName,BOOL AdjustColorsToVP,int displayImagePlaneInRed);
...
DWORD GM32NADCONINIT (char * InitFile);
...
BOOL getLAZMinMax(char * file, double * xmin, double * xmax, double * ymin, double * ymax, double * zmin, double * zmax);
