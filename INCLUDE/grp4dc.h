...
struct parameters {
	...
	HPSHORT reference_line;/* array of changing elements on reference line */
	HPSHORT coding_line;	/* array of changing elements on coding line */
};

struct decompressed_descriptor {
	HPSTR  data;				/* pointer to decompressed image */
...
struct compressed_descriptor {
	HPSTR  data;				/* pointer to compressed image */
	short  pixels_per_line;		/* the number of pixels in a scan line */
	short  number_of_lines;		/* the number of scan lines in the image */
