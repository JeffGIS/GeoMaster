...
Mosaic()
{
   ...
   // add the first image to the imagestage manager
   {
      ...
      ASSERT(LT_SUCCESS(src->initialize("data/r16.raw", pixelGrey, 64, 64)));
      LTIGeoCoord geo(10.0, 5.0, 1.0, -1.0, 0.0, 0.0, NULL);
      src->overrideGeoCoord(geo);
      ASSERT(LT_SUCCESS(manager->addImageStage(src)));
   }
   // add the second image to the imagestage manager
   {
      RC<LTIRawImageReader> src;
      ASSERT(LT_SUCCESS(src->initialize("data/g16.raw", pixelGrey, 64, 64)));
   ...
   ASSERT(LT_SUCCESS(mosaic->initialize(manager, NULL, false)));
      
   LTFileSpec outFS("x.raw");
   
   LTIRawImageWriter writer;
   ASSERT(LT_SUCCESS(writer.initialize(mosaic)));
   ASSERT(LT_SUCCESS(writer.setOutputFileSpec(outFS)));

   const LTIScene scene(0,0,154,109,1.0);
   
   ASSERT(LT_SUCCESS(writer.write(scene)));
}
