...
public:
   ...
   size_t pread(offset_type offset, void *buffer, size_t nbytes) const;
   size_t pwrite(offset_type offset, const void *buffer, size_t nbytes) const;
...
private:
   void privateInit(wchar_t *path, const char *mode);
#else
   char *m_path;
   int m_flags;
   int m_fd;
