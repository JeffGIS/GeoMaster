...
BOOL InitDdeStuff(HWND  In_handle)
{
 ...
 HGLOBAL LDA;
    ...
    if (!DdeInitialize(&idInst,(PFNCALLBACK) my_handle, APPCMD_FILTERINITS, NULL)) 
    {
        Hszize();
...
{  
   ...
   if (fBlockNextCB && !(wType & XTYPF_NOBLOCK)) 
    ...
    if (wType == XTYP_WILDCONNECT) 
    {
        ...
        if (hszItem != hszAppName && hszItem != NULL) return(0); 
    ...
    for (i = 0; i < CTOPICS; i++) 
    {
        if (DdeCmpStringHandles(topicList[i].hszTopic, hszTopic) == 0) 
        {
             ...
             */
            if (wType == XTYP_CONNECT) return(ValidateContext((PCONVCONTEXT)lData1));
...
HDDEDATA GWIZXfer(PXFERINFO pXferInfo, WORD iFmt)
{
  //  char szT[128];   // SS==DS!
    ...
    WORD i;
    char *ptr, poked[128];
    char *presponse, response[] = "Gwiz got your request";
    presponse = response;
    ptr = poked;
    ...
    case XTYP_ADVREQ:
        Delay(RenderDelay, FALSE);// RenderDelay initial set to zero
        if (!hDataCount[iFmt]) 
        {   // gets a global memory data handle
            ...
            if (pszData = DdeAccessData(hDataCount[iFmt], NULL))//lock the global handle 
