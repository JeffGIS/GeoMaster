...
extern "C" int wlanGetCurrentSSID(char * pSSID, int maxSSID, GUID *pInterfaceGuid)
{
	...
	*pSSID = 0;
	...
	else 
	{
		...
		if (pIfList->dwIndex < pIfList->dwNumberOfItems)
		{
					...
					*pInterfaceGuid = pIfInfo->InterfaceGuid;
					...
					if (dwResult == ERROR_SUCCESS)
					{
						if (pConnectInfo->wlanAssociationAttributes.dot11Ssid.uSSIDLength != 0)
						{
							...
							{
								pSSID[k] = (char)pConnectInfo->wlanAssociationAttributes.dot11Ssid.ucSSID[k];
							}
							pSSID[pConnectInfo->wlanAssociationAttributes.dot11Ssid.uSSIDLength] = 0;
						}
						for (k = 0; k < sizeof (pConnectInfo->wlanAssociationAttributes.dot11Bssid);k++)
						{
							DOT11_MAC_ADDRESS[k] = pConnectInfo->wlanAssociationAttributes.dot11Bssid[k];
							/*if (k == 5)
								wprintf(L"%.2X\n",
