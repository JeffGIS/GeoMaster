...
static char				LastSHPFile[128]=""; 
static long				SHPColor=-1;
static short			SHPWidth=0, SHPIndexType;  
static HFILE			FidSmallDBF=HFILE_ERROR;
static char				SmallDBFName[128]="";
...
static char				PGDBName[512]; 
static char				PGDBTable[66];   
static char				ShapeFieldName[66];   
static char				PGDB_SQL[512]=""; 
static char				PGDBFile[256];      
...
{   
	SHPFid = GSSiOpenFile (SHPFileName,NULL,OF_READ);
...
{   
	...
	if (!*SmallDBFName)
		GSSiGetTempFileName (NULL,"gms",NULL,(LPSTR)SmallDBFName);
...
{   
	...
	GSSiGetTempFileName (NULL,"gms",NULL,(LPSTR)TestName);
...
#endif
{   
	char	Name[128], str[260], Projection[34], Units[34];
...
//		ltoa (CurrentSHPRec,txt,10);
		StatusWindowUpdate (NULL,NULL,NumSHPRecs,CurrentSHPRec); 
...
{ 
	...
	if (!pDot)
		return FALSE;
	_fstrcpy (pDot,".shx");
	SHPIDXFid = GSSiOpenFile (Name,NULL,OF_READ);  
	...
	{ 
		...
		if (!pDot)
			return FALSE;
		_fstrcpy (pDot,".gsi");
		TMPFid = GSSiOpenFile (Name,NULL,OF_READ); 
		...
		else
		{
			...
			{
				HFILE SHPFid=GSSiOpenFile (SHPFileName,NULL,OF_READ); 
				
			    ii=_fstat (SHPFid,&stat);
...
long GetSHPRecordOffset (long record,BOOL UseBounds)
{
	...
	long	Len;
	long	MaxLen= (((long)UINT_MAX-100) / sizeof(SHPINDEXRECORD)) * sizeof(SHPINDEXRECORD); 
	LPSTR	pLoc, pLen;
		...
		case SHPT_POINT:  
			...
			{   
				...
				{   
					...
					CurView->MaxFileDisplayedPointWidth[FileNum] = max (CurView->MaxFileDisplayedPointWidth[FileNum],(size/ FileDistToWinDist)-(((long)CurrentItemMinMax.xmx)-CurrentItemMinMax.xmn));
...
DoPoly:
	        ...
	        BigRead (FidSHP,(HPSTR)pPartIndex,(long)nPoly*sizeof(long)); //pPartIndex[2]
	        ...
	        {   
	            ...
	            if (numpoints > (long)UINT_MAX)
	            	MessageBox (0,"Shape file record contains too many points",NULL,MB_ICONEXCLAMATION);
			...
			if (SHPHeader.ShapeType == SHPT_ARC || SHPHeader.ShapeType == SHPT_ARCM) 
			{
				...
				else if (CopyRec)  
				{
					...
					AddPolyToBuffer (nPoly,pNumPoints,&hPoints,TYPE_POLYLINE,CurrentRefno,NULL,-1,CurrentDesc,0,CurrentTAG,CurrentUDI,
...
//						} 
			...
			{   
				...
				{
					pNumPoints = (LPWORD)GlobalLock (hPolyPartLen);//pNumPoints[1]
					if (nPoly > 1)
					{
	                    ...
	                    GlobalUnlock (hPoints);
						AddPolyToBuffer (nPoly,pNumPoints,phPoly,TYPE_AREA,CurrentRefno,NULL,-1,CurrentDesc,0,CurrentTAG,CurrentUDI,
	                                     -1,-1,0,0,0,0,0,TRUE,&hUpdateBuf,&lUpdateBuf); 
		                for (i=0;i<nPoly;i++)
...
{ 
 ...
 switch(Message)
    ...
    case WM_INITDIALOG:
        ...
        *PGDBTable = 0;
        ...
        if (NumTAGDef)
		 ...
		 {
		 	Fid = GSSiOpenFile (str,NULL,OF_READ);
		 	if (Fid == HFILE_ERROR)
		 		goto LoadGSP;
		 	SHPType = ReadSHPHeader (Fid,&Bounds);  
		 	GSSiClose (Fid);
		 }  
		 ii = SHPType;
    ...
    case WM_COMMAND:
         ...
         {  
            case IDC_SHOW_FIELDS: 
            {
            	...
            	DisplayFieldList (hWndDlg,hSHPDBF,NULL,0); 
            	if (Opened) 
            	{
            	 	if (IsPGDB)
            	 		OpenPGDB (NULL,NULL,NULL);
            	 	else
						CloseDataFile (TRUE,&hSHPDBF); 
				}
            }	
   ...
   LoadGSP:
   				 {
   				 	HFILE	Fid = GSSiOpenFile (GSPName,NULL,OF_READ); 
   				 	short	SymNum=0;
						...
						if (*PGDBTable)
						{
							...
							SymNum = GetOrCreateSym (SymName,NULL,NULL,GetGlobalBVal2 ("[%ALLOWSYMBOLCREATION]",FALSE),type);  
   				 	...
   				 	if (fgetstring (str,128,Fid))
			         	SendDlgItemMessage (hWndDlg,IDC_PROJECTION,CB_SELECTSTRING,-1,(LPARAM)str);   
   				 	if (fgetstring (str,128,Fid))
            ...
            case IDOK: 
		        ...
		        if ((pDOT = _fstrrchr (GSPName,'.')))
		        {
		        	...
		        	break;
				Fid = GSSiOpenFile (GSPName,NULL,OF_CREATE);
				if (Fid == HFILE_ERROR)
				{   
...
BOOL OpenPGDBFileIndex (LPSTR DBNameIN,LPMNMXCORD WBounds)
{
    ...
    {   
		...
		SetPGDB_SQL (str);	
		return OpenPGDB (DBName,NULL,PGDB_SQL);
    } 
    else
...
long ListPGDBTables (LPSTR DBNameIN,HWND hWndDlg,UINT ListCntl,int ListType)
{
	...
	while (FetchDBRec (PGDBHandle))
	{
		...
		*ShpType = 0;  
		...
		SendDlgItemMessage ((HWND)hWndDlg,ListCntl,LB_ADDSTRING,NULL,(LPARAM) str); 
	}
	OpenPGDB (NULL,NULL,NULL); 
	for (i=0;i<Num;i++)
	{ 
        ...
        *pTab++ = 0;      
		...
		SendDlgItemMessage ((HWND)hWndDlg,ListCntl,LB_DELETESTRING,i,(LPARAM)NULL); 
		SendDlgItemMessage ((HWND)hWndDlg,ListCntl,LB_INSERTSTRING,i,(LPARAM) str); 
	}
	return Num;
}

BOOL FAR PASCAL SELECTPGDBMsgProc(HWND hWndDlg, WORD Message, WORD wParam, LONG lParam)
#if ENABLETRACE
{GSSiEnterProg (1107);
#endif
{   
         ...
         {  
            ...
            case IDOK: 
            {
				...
				{   
					...
					FidFL=GSSiOpenFile (FileList,NULL,OF_CREATE);
					for (i=0; i<nItems; i++,lpItems++)
					{   
			            SendDlgItemMessage(hWndDlg,IDC_LIST,LB_GETTEXT,*lpItems,(LPARAM)str);  
			            if ((pBS = _fstrchr (str,'\t')))
			            	*pBS=0;
			            sprintf (PGDBFile,"%s(%s)",File,str);
...
DoPoly:
	        ...
	        nPoly = SHPPolyHeader.NumParts;  
	        NumPoints = SHPPolyHeader.NumPoints; 
	        if (SHPPolyHeader.Type == SHPT_POLYGON || SHPPolyHeader.Type == SHPT_POLYGON_WITHCURVES || SHPPolyHeader.Type == SHPT_POLYGONZ || SHPPolyHeader.Type == SHPT_POLYGONM || SHPPolyHeader.Type == SHPT_PGDB_POLYGONZ)
	        	NumPoints = NumPoints+nPoly-1;
	        hPartIndex = GSSiGlobAlloc (1418,GMEM_MOVEABLE,sizeof(long)*(nPoly+1));
	        hPolyPartLen = GSSiGlobAlloc (1419,GMEM_MOVEABLE,sizeof(USHORT)*(nPoly+1));
			hPoints = GSSiGlobAlloc (1420,GMEM_MOVEABLE,sizeof(DPOINT)*NumPoints); 
	        pPartIndex = (HPLONG)GlobalLock (hPartIndex); 
	        hmemmove ((HPSTR)pPartIndex,&pRec[recloc],nPoly*sizeof(long));
	        ...
	        {   
	            ...
	            *pNumPoints++ = numpoints; 
	            ...
	            pPoints += numpoints;   
	            if (i && (SHPPolyHeader.Type == SHPT_POLYGON || SHPPolyHeader.Type == SHPT_POLYGON_WITHCURVES || SHPPolyHeader.Type == SHPT_POLYGONZ || SHPPolyHeader.Type == SHPT_POLYGONM))
	            	*pPoints++ = *pFirstPoint; 
	        } 
...
//					PickCurve (lpDCurPoints,nPnts,&BP,&POC,&EP);
			...
			if (SHPPolyHeader.Type == SHPT_ARC || SHPPolyHeader.Type == SHPT_ARCZ) 
			{
				if (Pick)
				{   
					PickPolylineD (pPoints,NumPoints,2,0,0,0);
