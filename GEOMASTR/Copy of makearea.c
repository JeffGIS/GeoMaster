...
static HANDLE	hLines=0;
static HANDLE	hNodes=0;
    ...
    case WM_COMMAND:
         ...
         {  
            ...
            case IDC_GET_INTERIOR_SYM:
            	 *str = 0;
                 if (!SelectLineSymbol (hWndDlg,1,str,NULL,NULL,FALSE))
                    break;  
            ...
            {
            	LPTHEME	pTheme;
            	short	pos, nareas, n, npoints, LineSymbol, IntLineSym, ExtLineSym, Type;  
            	long	iref, CurItem=0;
                LPSHORT	pPolyParts;
            	...
            	HANDLE	hPoints=GSSiGlobAlloc ( 588,GMEM_MOVEABLE,(long)16000*(long)sizeof(DPOINT));
            	HPDPOINT	pDPoint;
				...
				hHighlightArea = GetNextHighlightArea (0,NULL,&nHighlightAreaPoints);
		    	...
		    	if (!LinkBetweenNodes) 
				{   
					HANDLE	hNewPoints=GSSiGlobAlloc ( 589,GMEM_MOVEABLE,UINT_MAX);
					HPDPOINT	pNewPoints;
					char	TempFile[128];
					HFILE	FidTemp;
		   			USHORT	nNewPoints;
					    
					GSSiGetTempFileName (NULL,"gm",NULL,(LPSTR)TempFile);
					FidTemp = GSSiOpenFile (TempFile,NULL,OF_CREATE);
			        NumItems = BT_NUM_IN_INDEX(hHighlight);  
	                ...
	                if (!FileIsEditLayer && SendDlgItemMessage (hWndDlg,IDC_NEWMAP,BM_GETCHECK,0,0)) 
			    		CreateNewMap (PltName,&HLTBounds,NumSyms,hSymDesc,0,NULL,0,0,TRUE);
			    	else 
			    	{
			    	...
			    	while (ContinueProcessing && BigRead (FidTemp,(HPSTR)&nNewPoints,sizeof(nNewPoints))) 
			    	{
		                ...
		                if (PolyInHighlightArea (nNewPoints, hNewPoints))
							AddPolyToMap (1,&nNewPoints, &hNewPoints,Type,NewRef,NULL,2,LineSymbol,NULL,NULL,NULL,-1,-1,-1,0,0,0,0,TRUE);
		    ...
		    LastLoop:       
							...
							*pDPoint++ = ToPoint;
		                    ...
		                    if (PolyInHighlightArea (npoints,hPoints))
								AddPolyToMap (1,&npoints, &hPoints,Type,NewRef,NULL,2,LineSymbol,NULL,NULL,NULL,-1,-1,-1,0,0,0,0,TRUE);
							goto NextNode;
						}
						else
						{
...
{   
	...
	GSSiGetTempFileName (NULL,"gm",NULL,(LPSTR)File);
...
BOOL OpenRefConnectionFile (LPSTR InName,LPMNMXCORD pFileMNMX)
{   
	HANDLE	hDB; 
	OFSTRUCT	OFStruct;
	char	Name[128];   
	...
	if (!hDBRefConnect)                          
		return FALSE;
	GetLongPathName (Name,sizeof (Name));
	pBS = _fstrrchr (Name,'\\'); 
	pBS++;
	...
	hTranToLPoint =  STRAN2 (XFROM,YFROM,XTO,YTO,2,(LPFLOAT)&RSQMIN,1,NULL);
	return TRUE;
}
...
/*    GWDHead.NumIndexFields[1]=2;
	...
	
	{
		...
		GetFieldIDsFromNames (Name,&hKeyFields,NULL,"BPXFile;BPYFile");
		GWDAddIndex (Name,hKeyFields,FALSE);
		GSSiGlobFree (&hKeyFields);
		GetFieldIDsFromNames (Name,&hKeyFields,NULL,"EPXFile;EPYFile");
		GWDAddIndex (Name,hKeyFields,FALSE);
		GSSiGlobFree (&hKeyFields);
		GetFieldIDsFromNames (Name,&hKeyFields,NULL,"ConnectionStatus");
		GWDAddIndex (Name,hKeyFields,FALSE);
		GSSiGlobFree (&hKeyFields);
	} 
/*	
...
*/	
	...
	Vars[3].BT_VAROFF=12;
	BT_CREATE (Name, 4, FALSE, 4, 1,Vars,FALSE, 0, GWDHead.TimeStamp, FALSE);
...
{   
	...
	CloseRefConnectFile ();
	CreateRefConnectionFile ("[%CONSTATFILE].gmd");
	...
	nRecs = BT_NUM_IN_INDEX (hHighlight);
	CreateStatusWindow (CurView->hWnd,1,"Build Connection Table");
...
//		if (PickByRefno(Refno,NULL,NULL,-1))
    ...
    {   
    	...
    	nLoaded = 0;
		StatusWindowUpdate (NULL,"Check for Intersections", nBlocks, nLoaded);
    	for (iblock=0;iblock<nBlocks;iblock++)
    	{
			...
			StatusWindowUpdate (NULL,NULL, nBlocks, ++nLoaded);
			if (!ContinueProcessing)
				break;
	  	}
	}
...
{   
    ...
    Refno = LONG_MIN;
	nRecs = BT_NUM_IN_INDEX (lpGWDHead->BTHandle[0]);
	CreateStatusWindow (CurView->hWnd,1,"Fix Connections");
	while (ContinueProcessing && !BT_FIND (lpGWDHead->BTHandle[0],(LPSTR)&Refno,BT_FIRST,BT_GT,(LPSTR)&Offset)) 
	{   
		if (Refno == debugref)
			ii=1;
...
GotSym:
	if (pRC->NumPoints > 2)
	{
		if (!PickByRefno(pRC->Ref,NULL,NULL,-101))
		{
	    	MessageBox (GetFocus(),"refindex needs to be recreated",NULL,MB_ICONEXCLAMATION);
	        BlowOut(NULL,NULL);
	    }
   		...
   		if (hSavePoly)
		{
            ...
            {
	    		MessageBox (GetFocus(),"Num links exceeds max",NULL,MB_ICONEXCLAMATION); 
	    		return -1;
	    	}
            lpRect = (LPMNMXCORD) GlobalLock (hSavePoly);
            lpRect++;
        		...
        		ConnectKey.Point.y != StartPoint.y) 
    {
	    ...
	    {   
	    	...
	    	MessageBox (GetFocus(),mess,NULL,MB_ICONEXCLAMATION);
   			DisplayMarker (*pOpPointBase,5,NULL,0,0,0,FALSE,FALSE,NULL,NULL);
	    	return -1;
	    } 
	}
	First = FALSE;
...
#define	MAXCONNECT	64
    ...
    {   
		...
		GWDDeleteRecord (lpGWDHead,Offset); 
        PickByRefno (StartRef,NULL,NULL,-1);
...
Next:   
        ...
        {   
            ...
            {
		        PickByRefno (ConnectRef[i],NULL,NULL,-1);
...
{   
	...
	{
		*phLinkPoints = GSSiGlobAlloc (0,GMEM_MOVEABLE,sizeof(DPOINT)*(long)UINT_MAX);
		pLinkPoints = (HPDPOINT)GlobalLock (*phLinkPoints); 
...
#define	MAXCONNECT	64
	...
	if (!OpenRefConnectionFile ("[%CONSTATFILE].gmd",&CurView->FileMNMX))
		return FALSE;
	CloseRefConnectFile (); 
	if (!GetFieldIDsFromNames ("[%CONSTATFILE].gmd",&hKeyFields,NULL,"BPConnections"))
		return FALSE;
	rtn = GWDAddIndex ("[%CONSTATFILE].gmd",hKeyFields,FALSE);
	GSSiGlobFree (&hKeyFields);
    if (!rtn)
    	return FALSE;
	if (!GetFieldIDsFromNames ("[%CONSTATFILE].gmd",&hKeyFields,NULL,"EPConnections"))
		return FALSE;
	rtn = GWDAddIndex ("[%CONSTATFILE].gmd",hKeyFields,FALSE);
	GSSiGlobFree (&hKeyFields);
    if (!rtn)
	...
	if (!OpenRefConnectionFile ("[%CONSTATFILE].gmd",&CurView->FileMNMX))
		return FALSE; 
	rtn = FALSE; 
	GSSiGetTempFileName (NULL,"gmc",NULL,(LPSTR)LinkFile); 
	FidLinkFile = GSSiOpenFile (LinkFile,NULL,OF_CREATE);
    lpGWDHead = (LPGWDHEADER)GlobalLock (hDBRefConnect);
    pRC = (LPREFCONNECT)&lpGWDHead->GWDData;
    NumLinkPoints = 0;
	CreateStatusWindow (CurView->hWnd,1,"Create Connected Polygons");
    ...
    {  
		...
		nLoaded = 0;
		StatusWindowUpdate (NULL,"Step1", nRecs, nLoaded);
...
NextLink:
	...
	StatusWindowUpdate (NULL,"Step2", nRecs, nLoaded);
...
Exit:  
    ...
    if (CreateNewMap (OutFile,&CurView->WBounds,0,NULL,0,NULL,0,0,TRUE))
    {
		WORD	np;
		HANDLE	hPoints;
		HPDPOINT	pPoints; 
    	...
    	{   
    		...
    		BigRead (FidLinkFile,(HPSTR)&np,2); 
    		hPoints = GSSiGlobAlloc (0,GMEM_MOVEABLE,(long)np*sizeof(DPOINT)); 
    		pPoints = (HPDPOINT)GlobalLock (hPoints);
    		BigRead (FidLinkFile,(HPSTR)pPoints,(long)np*sizeof(DPOINT));  
    		GlobalUnlock (hPoints);
			AddPolyToMap (1,&np, &hPoints,1,Refno,NULL,2,SymNum,NULL,NULL,NULL,-1,-1,-1,0,0,0,0,TRUE);
			GSSiGlobFree (&hPoints);
		    AddToSymList (SymNum,&NumSyms,&hSymDesc);
			StatusWindowUpdate (NULL,"Step2", nRecs, GSSillseek (FidLinkFile,0,1));
		}
		AddSymToMap (NumSyms,hSymDesc,0,NULL); 
	    DestroySymList (&NumSyms,&hSymDesc);
	    CloseMap(TRUE);  
...
#define	MAXLOOPS	1024
	...
	POINT	Point;  
	LINKDATA	LinkData; 
	UINT	NumLoopPoints[MAXLOOPS], NumLoops;
	...
	HANDLE	hLinkPoints = GSSiGlobAlloc ( 578,GMEM_MOVEABLE,(long)UINT_MAX*sizeof(DPOINT));
	HPDPOINT	pLinkPoints;    
	OFSTRUCT	OFStruct;  
    BTVARDESC  Vars;  
    BOOL	JustDebugRec=FALSE,NoIslands=FALSE, StoreIslands=GetGlobalBVal2 ("[%STOREISLANDS]",TRUE); 
    BOOL	PromptForUnattached=FALSE;
    char	mess[128]; 
	long	AreaRef=1;     
	MNMXCORD	FileBounds;
	COLORREF	AreaColor;
	...
	CreateStatusWindow (CurView->hWnd,1,"Automatic Area Creation");
	...
	nRecs = BT_NUM_IN_INDEX (lpGWDHead->BTHandle[0]); 
	nLoaded = 0;
	StatusWindowUpdate (NULL,"Step1: Building Link Table", nRecs, nLoaded);
NextLink:
    pos = BT_FIRST; 
    cond = BT_GT;
    NumLinks = 0; 
	...
	{    
		pos = BT_NEXT;  
		cond = BT_ANY;  
		StatusWindowUpdate (NULL,NULL, nRecs,(nRecs - BT_NUM_IN_INDEX (lpGWDHead->BTHandle[0])));
		NumLinks++;
		if (Refno == LastRef)
			ii=1;    
		LastRef = Refno;
		...
		if (!(n%1000))
   		...
   		{   
   			...
   			*pLinkPoints++=ConnectPointBase; 
   			NewLinkPoints = TraceLink (UINT_MAX-1,Refno,LinkRef,StartRef,lpGWDHead,AtEnd,Offset,&pLinkPoints,&LinkData.ToNode,&LinkData.ToAZ,&NumLinkSyms,LinkSyms,FALSE);
   			if (NewLinkPoints < 0)
   			{
   			...
   			BigWrite (FidLinks,(HPSTR)&NumLinkPoints,2,-1);
	    ...
	    
    {
	    ...
	    while (!BT_FIND (hNodes,(LPSTR)&NodesKey,pos,BT_ANY,(LPSTR)&NodesData))
	    {
	    	...
	    	if (NodesKey.NodeID.x != LastNode.x || NodesKey.NodeID.y != LastNode.y)
	    	{
	    		...
	    		{
	    			BadNodes++; 
	    			DisplayMarker (NodesData.Point,4,NULL,0,0,0,FALSE,FALSE,NULL,NULL);
	    		}
	    		numlinks = 1;
	    		TotNodes++;
    ...
    {   
	    ...
	    CreateNewMap (Name,&CurView->WBounds,0,NULL,0,NULL,0,0,TRUE);
	    ...
	    while (BigRead (FidLinks,(HPSTR)&LinkRef,4)==4)
	    {
			...
			BigRead (FidLinks,(HPSTR)&NumLinkPoints,2);
   			pLinkPoints = (HPDPOINT)GlobalLock (hLinkPoints);  
			BigRead (FidLinks,(HPSTR)pLinkPoints,(long)NumLinkPoints * sizeof(DPOINT)); 
			GlobalUnlock (hLinkPoints);      			
			AddToSymList (LinkSyms[0],&NumSyms,&hSymDesc); 
			rtn=AddPolyToMap (1,&NumLinkPoints, &hLinkPoints,1,LinkRef,NULL,2,LinkSyms[0],NULL,NULL,NULL,-1,-1,-1,0,0,0,0,TRUE);
	    }
	    CloseMap(TRUE);  
		AddSymToMap (NumSyms,hSymDesc,0,NULL); 
        DestroySymList (&NumSyms,&hSymDesc);
	...
	nLoaded = 0;
	StatusWindowUpdate (NULL,"Step 2: Building Loop Table", nRecs, nLoaded);
...
FindNextLink:
	...
	nLoaded = 0;
	StatusWindowUpdate (NULL,"Final Step: Creating Polygons", nRecs, nLoaded);
    CreateNewMap (EditName,&FileBounds,0,NULL,0,NULL,0,0,TRUE);
	EditBounds = CurView->FileMNMX;
		...
		{   
			...
			if (BoundsInBounds (&OrigLoopData.MinMax,&LoopData.MinMax,1))
			{
				...
				{   
					...
					{
				 		MessageBox(GetFocus(), "Too many islands in area", NULL,MB_ICONEXCLAMATION);
						goto NoMoreExclusions; 
					}
					...
					{
						if (PickByRefno(LoopData.StoredAsInclusionRef,NULL,NULL,GetPickFile (-1)))
						{
							DeletePickedItem (0,12,92);
						}
					}
...
NoMoreExclusions:
		...
		if (!JustDebugRec || AreaRef == debugarearef)
			AddPolyToMap (NumLoops,NumLoopPoints,hLoopPoints,0,AreaRef,NULL,-1,NewAreaSymbol,stuff,NULL,NULL,AreaColor,-1,-1,0,0,0,0,TRUE);
		while (NumLoops--)
	...
	{   
        ...
        if (MessageBox( GetFocus(),mess,NULL,MB_YESNO|MB_ICONQUESTION)==IDYES) 
		{
			...
			{   
				...
				NumLoopPoints[0]=LoopData.NumSides;   
				if (n++ < nlast)
					AddPolyToMap (NumLoops,NumLoopPoints,hLoopPoints,0,LoopKey.LoopID,NULL,2,NewAreaSymbol,NULL,NULL,NULL,AreaColor,-1,-1,0,0,0,0,TRUE);
				while (NumLoops--)
					GlobalFree (hLoopPoints[NumLoops]);
			}
			{   
...
Exit:   
	...
	DisplayMarker (GotLink.Point,3,NULL,0,0,0,FALSE,FALSE,NULL,NULL);			
...
{   
	...
	BigRead (FidLoop,File,128);
	GSSiClose (FidLoop);
	GSSiRemove (File); 
	GSSillseek (FidLinks,0,0);
	BigRead (FidLinks,File,128);
	GSSiClose (FidLinks);
	GSSiRemove (File); 
...
{   
	...
	AddNodeFactor = pow (10,8 - ld);
	GSSiGetTempFileName (NULL,"gmn",NULL,(LPSTR)File);
	BTVar[0].BT_VARTYP=BT_INTEGER;
	BTVar[0].BT_VARLEN=4;
	BTVar[0].BT_VAROFF=0;
	BTVar[1].BT_VARTYP=BT_INTEGER;
	...
	GSSiGetTempFileName (NULL,"gml",NULL,(LPSTR)File); 
	...
	GSSiGetTempFileName (NULL,"gmb",NULL,(LPSTR)File); 
...
{   
	HFILE		FidBad;
	HPDPOINT	pPoint; 
	char		BadAreaFile[128];  
	UINT		np;  
	HANDLE		hLoop; 
	short		i,idesc=GetDictSymbolNumber ("H01");  
	HPEN		hPen,hOldPen;
			
	GetGlobalCVal ("[%BADAREAFILE]",BadAreaFile,"c:\\badarea.bin");
	FidBad = GSSiOpenFile (BadAreaFile,NULL,OF_READ);
...
HANDLE LoadLoopSides (LPUSHORT pNumSides,short NumLinks,long Offset,BOOL Reverse,LPSHORT pNumSym,LPSHORT pSymbols)
{   
	...
	if (*pNumSides == 0)
	{   
		MemLen = (long)sizeof(DPOINT)*(long)UINT_MAX;
...
GotSym:;
		...
		BigRead (FidLinks,(HPSTR)&np,2); 
		if ((long)(*pNumSides)+(long)(np-1) > (long)UINT_MAX)
		{
			HFILE		FidBad;
			HPDPOINT	pPoint; 
			char		BadAreaFile[128];
			...
			GlobalUnlock (hLoop);
			GlobalUnlock (hLoop2);
			MessageBox (0,"Area has more than 64K sides",NULL,MB_ICONEXCLAMATION);  
			GetGlobalCVal ("[%BADAREAFILE]",BadAreaFile,"c:\\badarea.bin");
			FidBad = GSSiOpenFile (BadAreaFile,NULL,OF_CREATE);
			BigWrite (FidBad,(HPSTR)pNumSides,2,-1);
			if (Reverse)
...
BOOL PointInLoop (DPOINT Point,long NumLoopPoints,HANDLE hLoopPoints)
{
	...
	rtn = POINT_IN_AREAD (Point, NumLoopPoints, pLoopPoints,NULL,NULL);
	GlobalUnlock (hLoopPoints);
	return rtn;
}

BOOL CalculateLoopSize (HFILE FidLoop,long Offset,short NumLinks,BOOL Reverse,
            			LPMNMXCORD pMinMax, LPDOUBLE pSize,LPUSHORT pNumSides, LPDPOINT pPoint1)
{   
	LOOPREC	LoopRec;
	BOOL	First=TRUE; 
	HPDPOINT	lpDPoints;
 ...
 switch (Message)
   {
    ...
    case GF_EXECUTE:  
    {
    	...
    	LPOINT	ConnectPointFile,OpEndFile;
    	DPOINT	ConnectPointBase,OpEndBase;
    	HANDLE	hLinkPoints = GSSiGlobAlloc ( 590,GMEM_MOVEABLE,(long)UINT_MAX*(long)8);
    	HPDPOINT	pLinkPoints;    
    	OFSTRUCT	OFStruct;  
   	    BTVARDESC  Vars;  
...
NextLink:
		...
		{    
			...
			if (!(n%1000))
       		...
       		{   
       			...
       			pLinkPoints = (HPDPOINT)GlobalLock (hLinkPoints);
       			*pLinkPoints++=ConnectPointBase; 
       			NewLinkPoints = TraceLink (UINT_MAX-1,Refno,LinkRef,StartRef,lpGWDHead,AtEnd,Offset,&pLinkPoints,&LinkData.ToNode,&LinkData.ToAZ,&NumLinkSyms,LinkSyms,TRUE);
       			if (NewLinkPoints < 0)
       			{
	       			GlobalUnlock (hLinkPoints);
					GlobalUnlock (hDBRefConnect);
	    ...
	    {   
		    ...
		    CreateNewMap (EditName,&CurView->WBounds,0,NULL,0,NULL,0,0,TRUE);
			EditBounds = CurView->FileMNMX;
		    _fstrcpy (PltName,EditName);   
		    GSSillseek (FidLinks,128,0);
		    while (BigRead (FidLinks,(HPSTR)&LinkRef,4)==4)
