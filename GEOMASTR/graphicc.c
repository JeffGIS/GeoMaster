...
BOOL GetGoogleMapFile(int type)
{
	...
	sprintf(cmd, "http://maps.googleapis.com/maps/api/staticmap?size=%ix%i&center=%f@,%f&sensor=false&zoom=%i&scale=%i&maptype=%s&key=%s", GoogleMapWidth, GoogleMapHeight, centerLL.y, centerLL.x, GoogleZoom, GoogleScale, mapType, GOOGLE_SERVER_KEY);
	ExpandText(cmd);
				...
				else
				{
					...
					else
					{
						...
						*phPoints = GSSiGlobalReAlloc(1801, *phPoints, sizeof(MNMXCORD)+*pnPnts*sizeof(DPOINT), GMEM_MOVEABLE);
						...
						{
							*phPolyPartLen = GSSiGlobAlloc(1803, GMEM_MOVEABLE, (nLoops + 2)*sizeof(int));
							pPolyParts = GlobalLock (*phPolyPartLen);
