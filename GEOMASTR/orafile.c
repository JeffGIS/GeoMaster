...
static char				LastORAFile[MAX_PATH]=""; 
...
#define MAXLEVELS	32
	...
	MaxQuads = USHRT_MAX / sizeof (QUADS);
	Quads[0].xmn = 0;
	Quads[0].ymn = 0;
	Quads[0].xmx = 255;
	Quads[0].ymx = 255;  
...
ProcessQuad:  
	if (!ContinueProcessing)
		goto Exit;
	StatusWindowUpdate2 (0,NumTotQuads,nQuadsProcessed++); 
...
HFILE SortORASpatially (HFILE Fid)
{
	...
	FidSorted = GSSiOpenFile (TempFile,0,OF_CREATE);
	...
	StatusWindowUpdate (0,"Step 1",ORAFileHeader.NumRecs,0);
	...
	{   
		...
		StatusWindowUpdate (0,0,ORAFileHeader.NumRecs,RecordNum);
    }
    if (!NumWithinSym)
    	XMid = 0;
    else
...
HFILE CreateORAFileIndex (HFILE FidORAFile,LPSTR IndexName,LPSTR ORAFileName,LPSTR Reason)
{   
	...
	if (FidORAFile == HFILE_ERROR)
		return FidORAFile; 
	DisableHalt = TRUE;
	sprintf (mess,"Creating Oracle Index:%s",Reason);
	...
	StatusWindowUpdate (0,"Step 2",ORAFileHeader.NumRecs,0);
...
S100:
		...
		StatusWindowUpdate (0,0,ORAFileHeader.NumRecs,i);
		if (!ContinueProcessing)
			break;
	}
	...
	{
		FidIdx = HFILE_ERROR;
		GSSiOpenFile (IndexName,0,OF_DELETE); 
	}
...
//		CloseDGNCellLibrary ();
	...
	if (ORAIDXFid == HFILE_ERROR)
		ORAIDXFid = CreateORAFileIndex (FidORAFile,Name,ORAFileName,"New"); 
	else if (GetGlobalLVal2 ("[%USERLEV]",1) > 2)
	{
	    struct _stati64 statIndex;
      ...
      if (errno == EBADF)
	    ...
	    rc = GSSifstat (FidORAFile,&statFile);
	    dtime = difftime (statIndex.st_mtime,statFile.st_mtime);  
	    if (dtime < 0)
		{
			char	reason[128];

			sprintf (reason,"Outdated by %f seconds",dtime);
			ORAIDXFid = CreateORAFileIndex (FidORAFile,Name,ORAFileName,reason);
		}
	}
	if (ORAIDXFid == HFILE_ERROR)
		return FALSE; 
	...
	{   
		...
		case ORAT_POINT:  
			...
			{   
				...
				if (UpdateItem == 8)
					Symnum = NewSymbol;
				AddPointToBuffer (CurPointLocD,CurrentRefno,0,Symnum,OrigSize, PTRot,0,hGRText,0,
		...
		case ORAT_ARC: 
			...
			goto DoPoly;
		case 2003:
		case 2007://ify for zoning areas
			ThreeD = FALSE;
			nOrdPerPoint = 2;
		case 3003:
		case ORAT_POLYGON:
					...
					case 1003: //outer ring
						...
						{ 
							...
							case 2:
							{
								...
								PT.y = pOrd[j+1];
								ConvertCoord(&PT,0,1);
								if (j + nOrdPerPoint >= ORARecordHeader.NumOrd)
									j+=nOrdPerPoint; 
								pp = &pPoints[i];
								if (Display)
									CurveFactor = 0;
...
GetOut:
			...
			if (Type == LINE) 
			{
				...
				else if (CopyRec)  
				{
					...
					if (UpdateItem == 8)
						Symnum = NewSymbol;
					if (UpdateItem == 61 && nUpdatePolyPoints)
					{
						GSSiGlobUlFree (&hPoints); 
						nPoints = nUpdatePolyPoints;   
						hPoints = hUpdatePoly; 
						pPoints = (HPDPOINT)GlobalLock (hPoints);  
						hUpdatePoly = 0;
					}
					AddPolyToBuffer (1,&nPoints,&hPoints,TYPE_POLYLINE,CurrentRefno,0,-1,Symnum,0,CurrentPrefix,CurrentUDI,
...
//						TempLineWidth = SHPWidth; 
			...
			else if (Type == ORATEXT)
				{
		        	...
		        	{   
						...
						ProcessTextObject (hDC,&GRTextHeader,txt,nchar,0,0,0,0,0);
						if (OldColor >= 0)
			...
			{   
				...
				else if (CopyRec)  
				{
					...
					AddPolyToBuffer (nPoly,pNumPoints,&hPoints,TYPE_AREA,CurrentRefno,0,-1,Symnum,0,CurrentPrefix,CurrentUDI,
                                     -1,-1,0,0,0,0,0,TRUE,&hUpdateBuf,&lUpdateBuf); 
                    GlobalUnlock (hPolyPartLen);
                }
				else
				{    
					if (Display)
					{
			        	...
			        	if (GetInVisibility(CurrentDesc))
			        	{
			        		if (CurView->HaveLayerColor[FileNum])
			        		{
								...
								hBorderPen = CreatePen (PS_SOLID,0,CurView->LayerColor[FileNum]);
					...
					if (PolyInMaskAreaFileCoord (GF_AREA,&NumPoints,0,0,&pPoints,TRUE))
					{
						...
						if (SDCrtn > 0)
							ProcessPolygon (hDC,ShowBorder,PltType,0,hBorderPen,0);
						else if (SDCrtn < 0)
							HaveTXLoc = TRUE;
...
{
	HFILE	Fid=GSSiOpenFile (ORAFileName,0,OF_READ);
	MNMXCORD Bounds;
...
{
	char	Name[MAX_PATH], str[260], Projection[128];
	short	l;
	HFILE	Fid; 
    LPSTR	pDot;
