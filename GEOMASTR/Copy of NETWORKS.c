...
static double	DistTOL=0.01;
static double	MarkerTOL=10;
...
long PointsBetweenPCT (long Refno,double FromPCT, double ToPCT,BOOL ShapePointsOnly,LPHANDLE phPoints,LPHANDLE phCurvePoints)
{   
	long	npnts;   
	BOOL	Reverse=FALSE;     
	HANDLE	hPoly;    
	HPDPOINT	OutPoints;  
	USHORT	NumOutPoints=0;  
...
{   
	...
	*phPoints = 0;
	...
	if (!OpenNetLinkAndRef (NetworkID,FALSE,&Opened))
		return 0;  
	*phPoints = GSSiGlobAlloc (0,GMEM_MOVEABLE,UINT_MAX);      
 ...
 switch (Message)
   {
   	case GF_INIT:
   		...
   		{
			FidChain = GSSiOpenFile ("NetChain.bin",&OFStruct,OF_READWRITE); 
...
NextChain:
		...
		if (*pChain == -2147436847)
			ii=1;			                                      
					PickByRefno(*pChain,NULL,NULL,-1);
					Sequence += 1000;     
			...
			break;
	GotHighlight:
			GetTrueStreetName (CurPath, CurrentStreetName, 0); 
	    	if (FidChain == HFILE_ERROR)
	     	{
			    	...
			    	{   if (FirstRef > LONG_MIN)
			    		{   
				    		...
				    		if (Dist1 > NetTOL) 
				    		{
				    			...
				    			HaveGaps = TRUE; 
				    			DisplayMarkers = TRUE;
				    			DisplayMarker (GapPoint,3,NULL,0,0,0,TRUE,FALSE,NULL,NULL);
	...
	//			    		MP += (NetDir * HighlightData1.PD.Length);
			    ...
			    if (Pass)
			    {
		 			...
		 			if (OppEndLinkID >= 0)
		 			{
			    		...
			    		{
							PickByRefno(NetRefsData.Ref,NULL,NULL,-1);
							Sequence += 1000;     
		 		...
		 		else
		 		{
		 			...
		 			{   
		 				...
		 				sprintf (mess,"This section contains one or more gaps of up to %f base units. Do you want the system to close them?",
		 						 MaxGap);
		 				if (GetGlobalBVal2 ("[%FIXNETGAPS]",FALSE) ||
		 					MessageBox (hWnd,mess,NULL,MB_YESNO) == IDYES)
		 				{
		 					FixGap = MaxGap + 0.01;      
		 					HaveGaps = FALSE; 
				...
				NextNet:
			    		...
			    		if (!PickByRefno (NetRefsData.Ref,NULL,NULL,-1))
			    		{
			    			MessageBox(hWnd,"Cannot locate existing network segments. Rebuild reference index",NULL,MB_ICONEXCLAMATION);
		 					CloseNetLinkAndRef (Opened); 
			    			goto Exit;
			    		} 
 ...
 switch (Message)
   {
		    ...
		    while (GetRouteAndMP (&pRandMP,Route,&MPVal))
		    {
			    ...
			    {   
					...
					{   
				    	...
				    	GetTrueStreetName (NetLinksKey.Path,TrueName,0);
	...
	NextStreet:;
			...
			DisplayMarkers = TRUE;
			DisplayMarker (MPPoint,2,txt,0.12,AZ+HALFPI,0,FALSE,FALSE,NULL,NULL);
			DisplayMarkers = FALSE;           
NextMarker:;
			CurLoc++; 
			if (Function == GF_LOAD_MARKERS_HLT)         
...
{     
	...
	LastPath:
		...
		DisplayMarkers = TRUE;
		DisplayMarker (NetIntRefData.Coord,3,NULL,0,0,0,TRUE,FALSE,NULL,NULL);
		DisplayMarkers = FALSE;           
NextInt:;          
	}
	GSSiSetCursor (hcurSave);
	 		...
	 		NetLinksKey.Ref = PickList[NumPicked-1].Refno;
	 	 	...
	 	 	{  
				...
				if (GetMilePointFromMP (NetLinksKey.Path,1,NetLinksData.MP, &NetMarker))
					...
					str2[0]=0; 
				_fstrcat (str,str2);
				SendDlgItemMessage (hWndDlg,IDC_NET_INFO,LB_ADDSTRING,NULL,(LPARAM)str); 
		 	 	st = BT_FIND (hBTNetLinks,(LPSTR)&NetLinksKey,BT_NEXT,BT_ANY,(LPSTR)&NetLinksData); 
		 	 }  
		 	 CloseNetLinkAndRef (Opened);
 ...
 switch (Message)
   {
   	case GF_INIT:
   		if (!CurPath)
   		{
   			MessageBox (hWnd,"No current street name",NULL,MB_ICONEXCLAMATION);  
   		}
   		else
   		{
	    	GetTrueStreetName (CurPath,TrueName,0);
    ...
    case WM_COMMAND:
    	if (wParam >= 32000)
    	{
    		...
    		{   
				...
				DrawStreet (CurPath,CurLinkID,RGB(255,0,255),3,TRUE,NULL,0);
    		}
           	PostMessage(hWnd, GF_CLOSE,0, 0L);
           	return TRUE;
    	}
    ...
    case WM_LBUTTONUP:
        ...
        {   
			...
			if (i) 
			{
				...
				TrackPopupMenu (PathMenu,TPM_RIGHTBUTTON,position.x,position.y,NULL,hWnd,NULL);
			}
...
{  
	...
	if (!GetMilePointFromFrame2 (NetDiskID,Frame,Path,MarkerID,NetMarker,pVidDir))
		return FALSE;
   	GetTrueStreetName (*Path,Name,0);
   	if (PathIn != *Path)
   	{ 
   		if (GetTrueStreetName (PathIn,TestName,0))
   		{
...
void AddIntersect (DPOINT AtPoint,long AtRef,short NumRefs,LPLONG IntRefs,
...
{   
    ...
    if (!hBTChained)
    	return;    
    ChainID++;
    hChain1 = GSSiGlobAlloc ( 533,GMEM_MOVEABLE,UINT_MAX);
    hChain2 = GSSiGlobAlloc ( 534,GMEM_MOVEABLE,UINT_MAX);
    if (pJoinRef)
    {
    	hChain = hChain1;
    	AtRef = *pJoinRef;  
...
NotBad:   
		...
		if (!BT_FIND (hBTNetVideo2,(LPSTR)&NetVidKeyTest,BT_FIRST,BT_GE,(LPSTR)&NetVidDataTest))
		{
			...
			{   
				...
				GetTrueStreetName (NetVidDataTest.Path, OverlapName, 0); 
...
Next:
	...
	GetTrueStreetName (Path, StreetName, 0); 
	...
	pos = BT_FIRST; 
 	...
 	{	 
 		...
 		{   
 			if (DupNames)
 			{   
 				GetTrueStreetName (DupNames, StreetName, 0); 
		...
		GetTrueStreetName (NetLinksKey.Path,Names[NumNames],0);
...
NextInt:  
	...
	CreateStatusWindow (hWndMain,1,"Dumping Intersections to Text File");
...
long GetStreetSegsBetweenMPs (HANDLE hDBDestSegs,long OnStreet,STREETMP FromMP,STREETMP ToMP,LPDPOINT FromPoint,LPDPOINT ToPoint,LPSTR Value)
{
	...
	NETREFINTKEY	NetRefIntKey;
	NETREFINTDATA	NetRefIntData;  
	HANDLE	hSegs=GSSiGlobAlloc (0,GMEM_MOVEABLE,UINT_MAX); 
...
void DumpIntersectionStreets (LPSTR Name)
{   
	BOOL	OpenedInt, OpenedSeg;
	long	LastInt=LONG_MAX;
	short	pos=BT_FIRST, nStreets, i, j;
	OFSTRUCT	OFStruct;
	HFILE	Fid;
	long	StreetNums[16]; 
	char	StreetName[80];
	char	str[1024]="IntID,X,Y,Street1,Street2,Street3,Street4,Street5,Street6,Street7,Street8,Street9,Street10,Street11,Street12,Street13,Street14,Street15,Street16";
    SEGDATAGM	Segdata; 
	HCURSOR	hcurSave; 
	DPOINT	NetCoord;
	...
	{   
		...
		{   
			if (LastInt < LONG_MAX)
			{   
				sprintf (str,"%ld,%f,%f",LastInt,NetCoord.x,NetCoord.y);
				for (i=0;i<nStreets;i++)
				{
					if (GetTrueStreetName (StreetNums[i],StreetName, 0)) 
