...
BOOL	ProcessSingleItem=FALSE, HaveFirstHeader;
float	CurPointSize; 
POINT	CurPointLoc;
...
HANDLE	hCoords=0; 
long	FastPickFileNum=-1, FastPickFII=-1; 
long	ParcelLoc=0;
BOOL	FastPick=FALSE;
	            ...
	            case 5: /* put area */
		 		...
		 		{   ipen = *ipnt; 
				    ...
				    if (hCoords) 
				    {
					    ...
					    pCoords += lCoords;
					    BufWrite (&pCoords,&lCoords,lpCurPoints,(long)nPnts*4);  
					    GlobalUnlock (hCoords);
					    lpCurPoints = GlobalLock (hCoords);
					    nPnts = nCoords;
				    }
		 		    ...
		 		    {   HPEN hSavePen=0;
						...
						{	
							if (!hTempPen || nPoly)
							{
								if (GetBit (7,&CurVis->WantType[7]) || GetBit (6,&CurVis->WantType[7]) || !ipen || !pens[ipen])
								{
									hSavePen = SelectObject(hDC, h0Pen);
								}
								else
								{
									hSavePen = SelectObject(hDC, pens[ipen]);
								}  
							}
							else 
							{
								if (GetBit (5,&CurVis->WantType[7]))    
						...
						if (nPoly)
						{
							...
							pCoords += ((long)nPolyPoints*(long)sizeof(POINT));
							pPartLen = GlobalLock (hPolyPartLen);
							pPartLen+=iPoly;
							*pPartLen = nPnts;
							GlobalUnlock (hPolyPartLen); 
							BufWrite (&pCoords,&lbuf,lpCurPoints,(long)nPnts*sizeof(POINT)); 
							if (!iPoly)
						...
						{    
							if (nPoly)
							{
								lpCurPoints = GlobalLock (hPolyBuffer);    
								nPnts = nPolyPoints;
							} 
							nCurPoints = nPnts;
							if (FileMode)
							{	
								...
								{   
									dPoint = FilePtToBasePt (*lpPoints4);
							...
							else 
							{
								...
								{	if (SetDisplayChar (hDC,GF_AREA,CurrentRefno,CurrentDesc,CurrentTAG,CurrentUDI))
									{
										if (FillAreas && (GetBit (7,&CurVis->WantType[7]) ||
														  GetBit (6,&CurVis->WantType[7])))
										{   
											if (Highlight)
											{
												...
												GWPolygon (hDC,lpCurPoints,nPnts);  
												SelectObject (hDC,hOldBrush);
												DeleteObject (hBrush);
											}
											GWPolygon (hDC,lpCurPoints,nPnts);  
											if (hTempPen && GetBit (5,&CurVis->WantType[7]) && nPoly)
											{   
												short	i;
	            ...
	            case 6: /* put polyline */
		 		...
		 		{   nPnts = *ipnt;
				    ...
				    if (hCoords) 
				    {
					    ...
					    pCoords = GlobalLock (hCoords);
					    pCoords += lCoords;
					    BufWrite (&pCoords,&lCoords,lpCurPoints,nPnts*4);  
					    GlobalUnlock (hCoords);
					    lpCurPoints = GlobalLock (hCoords);
					    nPnts = nCoords;
				    }
					...
					if (Visible & !AlreadyProcessed)
		 		    {
						...
						{
							if (Pick)
								PickPolyline (lpCurPoints,nPnts,2);
							else
							{
		...
		ProcessBaseRec: 
				...
				{	HFONT	hFont, OldFont; 
				    ...
				    if (CurView->PassID || Pick)
				    {
						...
						if (LogFont.lfHeight)
						{
							TSize = THeight / FileDistToBaseDist;  
		                	TextIsVisible = GetTextVisibility (TSize);
		                }
		                else
		                	TextIsVisible = FALSE;
...
//					ProjectFilePt (lpPoints); DisplayPointItem converts to windows coord
					...
					{
						AlreadyProcessed=TRUE;
						PickPointItem (*lpPoints,CurPointSize*ThemeWidthFactor,*pRot,CurrentDesc);			 		    
					} 
					else if (Visible)
					{
						AlreadyProcessed = TRUE; 
						...
						if (SetDisplayChar (hDC,GF_POINT,CurrentRefno,CurrentDesc,CurrentTAG,CurrentUDI))
							DisplayPointItem (hDC,CurPointLoc,CurPointSize*ThemeWidthFactor,*pRot,CurrentDesc);
					}
	        		TXLoc = CurPointLoc; 
	        	...
	        	{   
	        		...
	        		if (Pick || !hDC)
	        			break;
					if (SolidAreas && GetBit (7,&CurVis->WantType[7]))
...
int GWPolygon (HDC hDC, HPPOINT lpPoints, unsigned short npnts)
{	HANDLE Handle;
	HPPOINT	lpNewPoints, lpPntNew, lpPointsIn=lpPoints;
	unsigned short	i; 
	short	NewPnts, Attempts=0;  
	BOOL	ReduceNumPoints=FALSE;
	POINT	LastPoint;
	double	SkipDist = 0.1/(FileDistToWinDist*CurView->FileFactor);
...
TryAgain:
		...
		if (NewPnts > SHRT_MAX/2 && Attempts < 8)
		{
			SkipDist*=2; 
			Attempts++;    
			lpPoints = lpPointsIn;
...
{   
	...
	if (!hCurPolyPoints)
		return FALSE; 
	lpPoints = GlobalLock (hCurPolyPoints);
	FileNum = -1;
	rtn = PickPolylineD (lpPoints,nCurPolyPoints,1);
...
{   POINT	PickPoint;
    ...
    if (GetGlobalBVal ("[%PICKPERIM]"))
    	return (PickPolyline (lpPoints,nPnts,3));	
	...
	{   
		...
		PickListAdd (FileNum,CurView->SubFile,FileInIndex,ItemSeg,CurrentRefno,CurrentDesc,CurrentiPen,
					   Type,0,Offdist,0,Perim,CurrentItem,CurElement,BeginPoint,EndPoint,PickedPoint,
					   BeginPoint,0,
					   fabs(Area),&Rect,
...
BOOL PickPolyline (LPPOINT lpPoints,int nPnts, short Type)
{
	HANDLE Handle;
	LPPOINT	lpNewPoints, lpPntNew;
...
Add: 
	...
	PickListAdd (FileNum,CurView->SubFile,FileInIndex,ItemSeg,CurrentRefno,CurrentDesc,CurrentiPen,
				   ...
				   BeginPoint,EndPoint,PickedPoint,NearPoint,mini,0,&Rect,
				   BeginPointFile, EndPointFile,PickedPointFile,nPnts);
...
BOOL PickPolylineD (HPDPOINT lpPointsIn,short nPnts, short Type)
{
	DPOINT	BeginPoint, EndPoint, PickedPoint, WPoint, LastWPoint, NearPoint; 
	HPDPOINT lpPoints = lpPointsIn;
	POINT	BeginPointFile, EndPointFile, PickedPointFile;
	int	i, mini;
	double	PCT, OffDist, TotDistW, AZ, MinDist, Dist;
	BOOL	First=TRUE, rtn=FALSE;   
	MNMXCORD	Rect;
...
//    	return (PickPolyInArea(1,lpPoints,nPnts));
	...
	if (!LineInBoundsD (nPnts,lpPointsIn,TotDistW,&PCT,&OffDist,&AZ,
...
void PickListAdd (int FileNum,int SubFile, short FileInIndex,long CurrentSeg,
				  ...
				  short NearPoint,double Area, LPMNMXCORD Rect,
...
{	int i;
	...
	hOrthos = (HANDLE) GSSiGlobAlloc (GPTR,MAXORTHOBUFS*(long)sizeof(ORTHO));
	UsedOrthoBufs = 0;
	CurOrtho = (LPORTHO) GlobalLock (hOrthos);
...
*/	{
		if (!CurOrtho->hDib)
		{
		    ...
		    DIBColorsArePalleteEntries = ConvertDIBToPallete (&CurOrtho->hDib,CurOrtho->BitCount);
		}			
		if (CurOrtho->hDib)
		{
//            long nc = NumDIBColors (CurOrtho->hDib);
			DisplayBMInVP (CurView->hDC, CurOrtho->hDib, DIBColorsArePalleteEntries);
		}
	}
...
{    
	LPBITMAPINFO pBi=GlobalLock(hDib);
...
BOOL DisplayBMInVP (HDC hDC, HANDLE hDib, BOOL DIBColorsArePalleteEntries)
...
BOOL AddAreaToOffsetFile (int np, HPDPOINT lpDPoint)
{
	...
	if (!hAreaOffFile)
	{
		hAreaOffFile = GSSiGlobAlloc (GMEM_MOVEABLE,128);
...
HANDLE	GetConnectedItems (long StartSequence, LPLONG pEndSequence, LPSHORT pNumPoints)
{
	HPDPOINT	pPoints;
	short	pos;
	HANDLE	hPoints;   
    ...
    *pNumPoints = 0;    
	...
	{ 
		...
		{
			D1 = ldistp (OpenEnd,HighlightData1.PD.BeginPoint); 
			D2 = ldistp (OpenEnd,HighlightData1.PD.EndPoint); 
			if (D1 < D2)
				Reverse2 = FALSE;
			else
				Reverse2 = TRUE;
 ...
 switch (Message)
   {
    ...
    case WM_LBUTTONUP:
        ...
        if (NumPicked > 0)
        {
	        DoPaint=FALSE; 
	        str[0]=0;
           	if (GetTextString (hWnd,str,sizeof(str),"Enter new pen number",NULL))
        	{   
        		int	newpen;
...
void AdjustFileBounds (short VPID, short FileNo, LPMNMXCORD FileBounds)
{   
	DPOINT	DPoint;
    ...
    {   
        FileBounds->xmn += OrthoAdjustX;
        FileBounds->xmx += OrthoAdjustX;
        FileBounds->ymn += OrthoAdjustY;
        FileBounds->ymx += OrthoAdjustY; 
    }
	return;
}