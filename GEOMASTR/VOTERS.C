				...
				if ((pTAB = _fstrchr (str,'\t')))
				{
					...
					*pNum++ = 0;
					...
					_fstrcpy (UserListTitle[NumUserLists],str);
	           		UserListValues[NumUserLists]=GSSiGlobAlloc(GAIDNO 1513,GMEM_MOVEABLE,4+(1+(long)UserListMaxValueLength[NumUserLists])*UserListNumValues[NumUserLists]);
						...
						*pNewUserNumValues = SendDlgItemMessage (hWndDlg,IDC_VALUELIST,LB_GETCOUNT,0,0);
						if (!*pNewUserNumValues)
						 ...
						 if (!NewUserFieldIsNew)
           				 ...
           				 *phNewUserFieldValueList=GSSiGlobAlloc(GAIDNO 1514,GMEM_MOVEABLE,4+(1+(long)*pNewUserFieldMaxValueLength)*(*pNewUserNumValues));
						 pValue = GlobalLock (*phNewUserFieldValueList);
