...
{
	HANDLE hStr = GSSiGlobAlloc(GAIDNO 1469,GHND,len*5+3);
 ...
 switch(Message)
   {
	...
	case WM_TIMER:
		...
		{ 
			...
			hTmp = GSSiGlobAlloc(GAIDNO 1471,GMEM_MOVEABLE,lTmp+1);
...
BOOL RestartSocket (SOCKET sock)
{
	...
	if (i >= 0 && hSocketRestartString[i])
	{
    	...
    	if (lMacro)
    	{
    		HANDLE	hMacro = GSSiGlobAlloc(GAIDNO 1472,GMEM_MOVEABLE,lMacro+1);
...
Restart:
			if (i >= 0 && hSocketRestartString[i])
			{
    			...
    			if (lMacro)
    			{
    				HANDLE	hMacro = GSSiGlobAlloc(GAIDNO 1473,GMEM_MOVEABLE,lMacro+1);
    				LPSTR	pMacro = GlobalLock (hMacro);
    				_fstrcpy (pMacro,pRestartString);  
					GlobalUnlock (hSocketRestartString[i]);
    				GlobalUnlock (hMacro);  
...
GotID:	
	...
	hSocketBuffer[i] = GSSiGlobAlloc(GAIDNO 1474,GHND,MAX_SOCKET_BUFFER_SIZE);
	if (UpdateServer)
	{
		LenSocketInputTerminator[i] = 1;
		strcpy (SocketInputTerminator[i],"");  
...
GotID:	
	...
	*pSocket = sock;
	...
	hSocketBuffer[i] = GSSiGlobAlloc(GAIDNO 1475,GHND,MAX_SOCKET_BUFFER_SIZE);
...
//				lRec++;
			...
			*&pBuffer[LenSocketBuffer[i]] = 0;
			...
			{ 
				...
				lTmp = strlen (pCommand);
				hTmp = GSSiGlobAlloc(GAIDNO 1476,GMEM_MOVEABLE,lTmp+1);
				pTmp = GlobalLock (hTmp);
				strcpy (pTmp,pCommand);
				GlobalUnlock (hSocketProcessString[i]);
				GlobalUnlock (hSocketBuffer[i]);
			...
			{   
				...
				*CmdMess = 0;  
				...
				{
					HANDLE	hCmd = GSSiGlobAlloc(GAIDNO 1477,GMEM_MOVEABLE,lCmd+1);
...
{
	HANDLE	hStr=GSSiGlobAlloc(GAIDNO 1478,GMEM_MOVEABLE,1024);
...
{    
	...
	UINT	WaitSeconds=20;      
	short	rtn=0;  
	HANDLE	hStr=GSSiGlobAlloc(GAIDNO 1479,GMEM_MOVEABLE,1024);
	LPSTR	str=GlobalLock (hStr);   
	...
	{   
		...
		{   
			hSegRec = GSSiGlobAlloc(GAIDNO 1480,GMEM_MOVEABLE,SegLen);
...
{   
    ...
    {   
    	...
    	{   
    		...
    		if (lMacro)
    		{
    			hMacro = GSSiGlobAlloc(GAIDNO 1481,GMEM_MOVEABLE,lMacro+1);
