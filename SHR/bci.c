...
__inline double td_func16(doublexyz xyz, const double *c);
__inline double td_firstxderfunc16(doublexyz xyz, const double *c);
__inline double td_firstyderfunc16(doublexyz xyz, const double *c);
__inline double td_mixedderfunc16(doublexyz xyz, const double *c);

void copyxyz(int width, const doublexyz *xyzfrom, doublexyz *xyz, int cols,
             int i, int j);
void copy4xyzwder(doublexyzwder *xyzwder, const doublexyz *xyz,
                  const doubleder *der, int cols, int x, int y);

__inline void countderivs(doubleder *der, const doublexyz *xyz, const double *c,
                 int cols, int x, int y);
void td_fillgrid(const doublexyz *xyz, int cols, int rows, doublexyz *fxyz,
                 int fcols, int frows);
void td_fillgrid_wo_nulls(const doublexyz *xyz, int cols, int rows,
                          doublexyz *fxyz, int fcols, int frows);
...
/* temporary check for square matrix */
    ...
    bzero(splxyz, 16*sizeof(doublexyz));
    der = (doubleder *)calloc(rows*cols, sizeof(doubleder));
