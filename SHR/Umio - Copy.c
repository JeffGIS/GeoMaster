...
Again:
	...
	{   
		...
		while (ContinueProcessing && !rtn && !WaitForInputOnSocketStatus && GetMessage(&msg,0,0,0))
		{   
			switch (msg.message)
			{   
				case WM_TIMER:
...
Top:
	...
	_fmemmove (&pBuffer[LenSocketBuffer[i]],SocketInputBuffer,err);
	*&pBuffer[LenSocketBuffer[i]+err] = 0; 
	LogSocketInput (i,SocketInputBuffer,err);     
	LenSocketBuffer[i]+=err; 
	DisplayLenSocketBuffer (i);
	while (!InDisplayProcessing && LenSocketBuffer[i])
	{   
