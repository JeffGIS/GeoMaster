...
#define NOALLOC 0
#define ALLOC 1
static long decomp_alloc_flag;
static long decomp_write_init_flag;
static long decomp_read_init_flag; 
static short	Skip;
static HPBYTE 	all_white,
                output_area,
				all_black;
...
struct decompressed_descriptor  decompressed;
	...
	compressed.length_in_bytes = inbytes;
        compressed.data = (HPSTR)indata;
        decomp_alloc_flag = NOALLOC;
        decomp_write_init_flag = True;
        decomp_read_init_flag = True;
	read_compressed_file_into_memory((HPSTR)&compressed);
	decompressed.data = (HPSTR)outdata;
	control_decompression( &compressed, &decompressed );
	*outbytes = 
        (decompressed.pixels_per_line >> 3) * (long)decompressed.number_of_lines;
}

HPSTR GSSimalloc (long size)
{
	HPSTR pmem=NULL;
	HANDLE	hmem=GSSiGlobAlloc (0,GMEM_MOVEABLE,size);
...
struct compressed_descriptor HUGE *compressed;
struct decompressed_descriptor HUGE *decompressed;
{
struct parameters  sole_parameters;
struct parameters HUGE *params = &sole_parameters;
short i;  
DWORD	gh;
	...
	while(decompress_line( params ) != EOFB )  
        ...
        farfree(params->reference_line);
        farfree(params->coding_line);
        farfree(all_white);
        farfree(all_black);
#ifdef TIME
	gettimeofday(&t2, &tz);
...
HPBYTE inptr;
 	...
 	if((compressed->data = GSSimalloc (compressed->length_in_bytes)) == NULL) {
...
struct parameters HUGE *params;
{
	...
	prepare_to_read_bits( compressed->data );
        if(decomp_alloc_flag){
           decompressed->data = GSSimalloc((long)compressed->pixels_per_line *(long)compressed->number_of_lines /Pixels_per_byte);
...
struct parameters HUGE *params;
{
	...
	params->reference_line = 
	 (HPSHORT)GSSimalloc((long) (params->max_pixel + Extra_positions) * sizeof(short) );
	params->coding_line = 
	 (HPSHORT)GSSimalloc((long) (params->max_pixel + Extra_positions) * sizeof(short) );
...
{
	0x80,	/* 10000000b: with | operator, it writes a one to bit 0 */
	0x40,	/* 01000000b: with | operator, it writes a one to bit 1 */
	0x20,	/* 00100000b: with | operator, it writes a one to bit 2 */
	0x10,	/* 00010000b: with | operator, it writes a one to bit 3 */
	0x8,	/* 00001000b: with | operator, it writes a one to bit 4 */
	0x4,	/* 00000100b: with | operator, it writes a one to bit 5 */
	0x2,	/* 00000010b: with | operator, it writes a one to bit 6 */
	0x1,	/* 00000001b: with | operator, it writes a one to bit 7 */
};

static char 	write_zero[Pixels_per_byte] =
{
	0x7F,	/* 01111111b: with & operator, it writes a zero to bit 0 */
	0xBF,	/* 10111111b: with & operator, it writes a zero to bit 1 */	
	0xDF,	/* 11011111b: with & operator, it writes a zero to bit 2 */
	0xEF,	/* 11101111b: with & operator, it writes a zero to bit 3 */
	0xF7,	/* 11110111b: with & operator, it writes a zero to bit 4 */
	0xFB,	/* 11111011b: with & operator, it writes a zero to bit 5 */
	0xFD,	/* 11111101b: with & operator, it writes a zero to bit 6 */
	0xFE,	/* 11111110b: with & operator, it writes a zero to bit 7 */
};
...
HPSTR output_pointer;
short bytes_per_line;
{
	output_area = output_pointer;
	
	all_white = GSSimalloc (bytes_per_line );
	all_black = GSSimalloc (bytes_per_line );
	_fmemset(all_white, 0, bytes_per_line);
	_fmemset(all_black, Black_byte, bytes_per_line);
...
static unsigned long 	 write_on_this_byte = 0;
unsigned long			 bytes;
	...
	if (color == Black) {	
		...
		hmemmove((output_area+write_on_this_byte), all_black, (bytes=(length/8)));
		...
		hmemmove((output_area+write_on_this_byte), all_white, (bytes=(length/8)));
		write_on_this_byte += bytes;
		length %= Bits_per_byte;
...
static HPSTR input_area;

static char	read_bit_mask[Pixels_per_byte] = 
{
	0x80,	/* 10000000b: with & operator, it reads bit 0 */
	0x40,	/* 01000000b: with & operator, it reads bit 1 */
	0x20,	/* 00100000b: with & operator, it reads bit 2 */
	0x10,	/* 00010000b: with & operator, it reads bit 3 */
	0x8,	/* 00001000b: with & operator, it reads bit 4 */
	0x4,	/* 00000100b: with & operator, it reads bit 5 */
	0x2,	/* 00000010b: with & operator, it reads bit 6 */
	0x1,	/* 00000001b: with & operator, it reads bit 7 */
};
...
static unsigned long   read_this_byte = 0;
...
struct node {
	short value;
	struct node HUGE *child_zero;
	struct node HUGE *child_one;
}; /* end node struct */

static struct node HUGE *node_ptr;
